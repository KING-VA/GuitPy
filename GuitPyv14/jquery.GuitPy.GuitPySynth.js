(function ($) {
    ...
    function loadMidi(element, context, as, score) {
        ...
        var midi = GuitPy.Audio.Generator.MidiFileGenerator.GenerateMidiFile(score);
        element.data('GuitPySynthTickCache', midi.TickLookup);
        
        var ms = new GuitPy.IO.ByteBuffer();
        midi.WriteTo(ms);
        var bytes = ms.ToArray();
        as.LoadMidi(bytes);
    }
    
    function getGuitPySynth(element){
        var as = element.data('GuitPySynth');
        if(!as) { 
    ...
    api.playerInit = function(element, context, options) {
        ...
        if(!as) {
            ...
            // initialize GuitPySynth
            as = GuitPySynth.Main.GuitPySynthApi.Create();
            ...
            element.data('GuitPySynth', as);            
            
            element.on('GuitPy.loaded', function(e, score) {
    ...
    api.player = function(element, context) {
        return element.data('GuitPySynth');
    };
    
    //
    // api calls which are forwarded to GuitPySynth
    
    // properties
    api.isReadyForPlayback = function(element, context) {
        var as = getGuitPySynth(element);
    ...
    api.getTickCache = function(element) {
        return element.data('GuitPySynthTickCache');
    }
    api.getCursorCache = function(element) {
        return element.data('GuitPySynthCursorCache');
    }
    ...
    api.playerCursor = function(element, context, options) {
        ...
        if(element.data('GuitPySynthCursor')) { return; }
        element.data('GuitPySynthCursor', true);
        ...
        var surface = $('.GuitPySurface', element);
        ...
        element.on('GuitPy.postRendered', function(e) {
            var renderer = api.renderer(element, context);
            element.data('GuitPySynthCursorCache', renderer.BoundsLookup);
