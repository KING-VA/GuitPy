...
var GuitPySynth = GuitPySynth || {};
GuitPySynth.Main = GuitPySynth.Main || {};
GuitPySynth.Main.GuitPySynthApi = function (){
};
GuitPySynth.Main.GuitPySynthApi.Create = function (GuitPySynthScriptFile){
    // var swf = SwfObject;
    ...
    if (!((GuitPySynthScriptFile==null)||(GuitPySynthScriptFile.length==0))){
        // append script name 
        if (!(GuitPySynthScriptFile.lastIndexOf(".js")==(GuitPySynthScriptFile.length-".js".length))){
            if (!(GuitPySynthScriptFile.lastIndexOf("/")==(GuitPySynthScriptFile.length-"/".length))){
                GuitPySynthScriptFile += "/";
            }
            GuitPySynthScriptFile += "GuitPySynth.js";
        }
        if (!GuitPySynthScriptFile.indexOf("http")==0 && !GuitPySynthScriptFile.indexOf("https")==0){
            var root = new Array();
            ...
            root.push(GuitPySynthScriptFile);
            GuitPySynthScriptFile = root.join('');
        }
    }
    else {
        GuitPySynthScriptFile = GuitPySynth.Platform.Platform.ScriptFile;
    }
    if (supportsWebAudio && !forceFlash){
        GuitPySynth.Util.Logger.Info("Will use webworkers for synthesizing and web audio api for playback");
        return new GuitPySynth.Main.GuitPySynthWebWorkerApi(new GuitPySynth.Main.GuitPySynthWebAudioOutput(), GuitPySynthScriptFile);
    }
    if (supportsWebWorkers){
        GuitPySynth.Util.Logger.Info("Will use webworkers for synthesizing and flash for playback");
        return new GuitPySynth.Main.GuitPySynthWebWorkerApi(new GuitPySynth.Main.GuitPySynthFlashOutput(GuitPySynthScriptFile), GuitPySynthScriptFile);
    }
    GuitPySynth.Util.Logger.Error("Incompatible browser");
    throw $CreateException(new System.Exception.ctor$$String("Incompatible browser"), new Error());
};
GuitPySynth.Main.GuitPySynthFlashOutput = function (GuitPySynthRoot){
    this._GuitPySynthRoot = null;
    this._id = null;
    this._swfId = null;
    ...
    this._GuitPySynthRoot = GuitPySynthRoot;
    var lastSlash = this._GuitPySynthRoot.lastIndexOf("/");
    if (lastSlash != -1){
        this._GuitPySynthRoot = this._GuitPySynthRoot.substr(0, lastSlash + 1);
    }
};
GuitPySynth.Main.GuitPySynthFlashOutput.prototype = {
    get_SampleRate: function (){
        return 44100;
    },
    Open: function (){
        this._playbackSpeed = 1;
        this._id = "GuitPySynthFlashPlayer" + GuitPySynth.Main.GuitPySynthFlashOutput.NextId;
        this._swfId = this._id + "swf";
        GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[this._id] = this;
        GuitPySynth.Main.GuitPySynthFlashOutput.NextId++;
        this._swfContainer = document.createElement("div");
        this._swfContainer.className = "GuitPySynthFlashPlayer";
        this._swfContainer.setAttribute("id", this._id);
        document.body.appendChild(this._swfContainer);
        var swf =  swfobject;
        var embedSwf = swf["embedSWF"];
        embedSwf(this._GuitPySynthRoot + "GuitPySynth.FlashOutput.swf", this._id, "1px", "1px", "9.0.0", null, {
            id: this._id,
            sampleRate: 44100
        }, {
    ...
    Play: function (){
        document.getElementById(this._swfId).GuitPySynthPlay();
    },
    Pause: function (){
        document.getElementById(this._swfId).GuitPySynthPause();
    },
    SequencerFinished: function (){
        document.getElementById(this._swfId).GuitPySynthSequencerFinished();
    },
    AddSamples: function (samples){
        ...
        document.getElementById(this._swfId).GuitPySynthAddSamples(b64);
    },
    ResetSamples: function (){
        document.getElementById(this._swfId).GuitPySynthResetSamples();
    },
...
$StaticConstructor(function (){
    GuitPySynth.Main.GuitPySynthFlashOutput.PreferredSampleRate = 44100;
    GuitPySynth.Main.GuitPySynthFlashOutput.Id = "GuitPySynthFlashPlayer";
    GuitPySynth.Main.GuitPySynthFlashOutput.Lookup = null;
    GuitPySynth.Main.GuitPySynthFlashOutput.NextId = 0;
    GuitPySynth.Main.GuitPySynthFlashOutput.Lookup = {};
});
GuitPySynth.Main.GuitPySynthFlashOutput.OnReady = function (id){
    if (GuitPySynth.Main.GuitPySynthFlashOutput.Lookup.hasOwnProperty(id)){
        GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[id].Ready();
    }
};
GuitPySynth.Main.GuitPySynthFlashOutput.OnSampleRequest = function (id){
    if (GuitPySynth.Main.GuitPySynthFlashOutput.Lookup.hasOwnProperty(id)){
        GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[id].SampleRequest();
    }
};
GuitPySynth.Main.GuitPySynthFlashOutput.OnFinished = function (id){
    if (GuitPySynth.Main.GuitPySynthFlashOutput.Lookup.hasOwnProperty(id) && GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[id].Finished != null){
        GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[id].Finished();
    }
};
GuitPySynth.Main.GuitPySynthFlashOutput.OnSamplesPlayed = function (id, samples){
    if (GuitPySynth.Main.GuitPySynthFlashOutput.Lookup.hasOwnProperty(id)){
        GuitPySynth.Main.GuitPySynthFlashOutput.Lookup[id].SamplesPlayed(samples);
    }
};
GuitPySynth.Main.GuitPySynthWebAudioOutput = function (){
    this._context = null;
    this._buffer = null;
    this._source = null;
...
GuitPySynth.Main.GuitPySynthWebAudioOutput.prototype = {
    get_SampleRate: function (){
        return this._context.sampleRate | 0;
    },
    Open: function (){
        this._finished = false;
        this._circularBuffer = new GuitPySynth.Ds.CircularSampleBuffer(40960);
...
$StaticConstructor(function (){
    GuitPySynth.Main.GuitPySynthWebAudioOutput.BufferSize = 4096;
    GuitPySynth.Main.GuitPySynthWebAudioOutput.BufferCount = 10;
});
GuitPySynth.Main.GuitPySynthWebWorker = function (main){
    this._player = null;
    this._main = null;
    this._main = main;
    this._main.addEventListener("message", $CreateDelegate(this, this.HandleMessage), false);
    this._player = new GuitPySynth.GuitPySynth();
    ...
    this._main.postMessage({
        cmd: "GuitPySynth.ready"
    });
};
GuitPySynth.Main.GuitPySynthWebWorker.prototype = {
    HandleMessage: function (e){
        var data = e.data;
        var cmd = data["cmd"];
        switch (cmd){
            case "GuitPySynth.setLogLevel":
                GuitPySynth.Util.Logger.LogLevel = data["value"];
                break;
            case "GuitPySynth.setMasterVolume":
                this._player.set_MasterVolume(data["value"]);
                break;
            case "GuitPySynth.setMetronomeVolume":
                this._player.set_MetronomeVolume(data["value"]);
                break;
            case "GuitPySynth.setPlaybackSpeed":
                this._player.set_PlaybackSpeed(data["value"]);
                break;
            case "GuitPySynth.setTickPosition":
                this._player.set_TickPosition(data["value"]);
                break;
            case "GuitPySynth.setTimePosition":
                this._player.set_TimePosition(data["value"]);
                break;
            case "GuitPySynth.setPlaybackRange":
                this._player.set_PlaybackRange(data["value"]);
                break;
            case "GuitPySynth.setIsLooping":
                this._player.set_IsLooping(data["value"]);
                break;
            case "GuitPySynth.play":
                this._player.Play();
                break;
            case "GuitPySynth.pause":
                this._player.Pause();
                break;
            case "GuitPySynth.playPause":
                this._player.PlayPause();
                break;
            case "GuitPySynth.stop":
                this._player.Stop();
                break;
            case "GuitPySynth.loadSoundFontBytes":
                this._player.LoadSoundFont(data["data"]);
                break;
            case "GuitPySynth.loadMidiBytes":
                this._player.LoadMidi(data["data"]);
                break;
            case "GuitPySynth.setChannelMute":
                this._player.SetChannelMute(data["channel"], data["mute"]);
                break;
            case "GuitPySynth.setChannelSolo":
                this._player.SetChannelSolo(data["channel"], data["solo"]);
                break;
            case "GuitPySynth.setChannelVolume":
                this._player.SetChannelVolume(data["channel"], data["volume"]);
                break;
            case "GuitPySynth.setChannelProgram":
                this._player.SetChannelProgram(data["channel"], data["program"]);
                break;
            case "GuitPySynth.resetChannelStates":
    ...
    OnPositionChanged: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.positionChanged",
    ...
    OnPlayerStateChanged: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.playerStateChanged",
            state: e.State
        });
    },
    OnFinished: function (isLooping){
        this._main.postMessage({
            cmd: "GuitPySynth.finished",
            isLooping: isLooping
        });
    },
    OnSoundFontLoaded: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.soundFontLoaded"
        });
    },
    OnSoundFontLoadFailed: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.soundFontLoadFailed"
        });
    },
    OnMidiLoaded: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.midiLoaded"
        });
    },
    ...
    OnReadyForPlayback: function (sender, e){
        this._main.postMessage({
            cmd: "GuitPySynth.readyForPlayback"
        });
    },
...
$StaticConstructor(function (){
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPrefix = "GuitPySynth.";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdInitialize = "GuitPySynth.initialize";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetLogLevel = "GuitPySynth.setLogLevel";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetMasterVolume = "GuitPySynth.setMasterVolume";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetMetronomeVolume = "GuitPySynth.setMetronomeVolume";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetPlaybackSpeed = "GuitPySynth.setPlaybackSpeed";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetTickPosition = "GuitPySynth.setTickPosition";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetTimePosition = "GuitPySynth.setTimePosition";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetPlaybackRange = "GuitPySynth.setPlaybackRange";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetIsLooping = "GuitPySynth.setIsLooping";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPlay = "GuitPySynth.play";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPause = "GuitPySynth.pause";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPlayPause = "GuitPySynth.playPause";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdStop = "GuitPySynth.stop";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdLoadSoundFontBytes = "GuitPySynth.loadSoundFontBytes";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdLoadMidiBytes = "GuitPySynth.loadMidiBytes";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetChannelMute = "GuitPySynth.setChannelMute";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetChannelSolo = "GuitPySynth.setChannelSolo";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetChannelVolume = "GuitPySynth.setChannelVolume";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSetChannelProgram = "GuitPySynth.setChannelProgram";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdResetChannelStates = "GuitPySynth.resetChannelStates";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdReady = "GuitPySynth.ready";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdReadyForPlayback = "GuitPySynth.readyForPlayback";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPositionChanged = "GuitPySynth.positionChanged";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdPlayerStateChanged = "GuitPySynth.playerStateChanged";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdFinished = "GuitPySynth.finished";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSoundFontLoaded = "GuitPySynth.soundFontLoaded";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdSoundFontLoadFailed = "GuitPySynth.soundFontLoadFailed";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdMidiLoaded = "GuitPySynth.midiLoaded";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdMidiLoadFailed = "GuitPySynth.midiLoadFailed";
    GuitPySynth.Main.GuitPySynthWebWorker.CmdLog = "GuitPySynth.log";
    if (!self.document){
        var main = self;
        main.addEventListener("message", function (e){
            ...
            var cmd = data["cmd"];
            switch (cmd){
                case "GuitPySynth.initialize":
                    GuitPySynth.Player.WebWorkerOutput.PreferredSampleRate = data["sampleRate"];
                    new GuitPySynth.Main.GuitPySynthWebWorker(main);
                    break;
            }
        }, false);
    }
});
GuitPySynth.Main.GuitPySynthWebWorkerApi = function (player, GuitPySynthScriptFile){
    this._synth = null;
    this._output = null;
    this._events = null;
    ...
    this._state = GuitPySynth.PlayerState.Paused;
    this._logLevel = GuitPySynth.Util.LogLevel.None;
    ...
    this._output.Open();
    try{
        this._synth = new Worker(GuitPySynthScriptFile);
    }
    catch($$e1){
        // fallback to blob worker 
        try{
            var script = "importScripts(\'" + GuitPySynthScriptFile + "\')";
            var blob = new Blob([script]);
            this._synth = new Worker(window.URL.createObjectURL(blob));
        }
        catch(e){
            GuitPySynth.Util.Logger.Error("Failed to create WebWorker: " + e);
...
GuitPySynth.Main.GuitPySynthWebWorkerApi.prototype = {
    ...
    get_LogLevel: function (){
        return GuitPySynth.Util.Logger.LogLevel;
    },
    set_LogLevel: function (value){
        GuitPySynth.Util.Logger.LogLevel = value;
        this._synth.postMessage({
            cmd: "GuitPySynth.setLogLevel",
            value: value
        });
    },
    ...
    set_MasterVolume: function (value){
        value = GuitPySynth.Synthesis.SynthHelper.ClampF(value, 0, 10);
        this._masterVolume = value;
        this._synth.postMessage({
            cmd: "GuitPySynth.setMasterVolume",
            value: value
        });
    },
    ...
    set_MetronomeVolume: function (value){
        ...
        this._synth.postMessage({
            cmd: "GuitPySynth.setMetronomeVolume",
            value: value
        });
    },
    ...
    set_PlaybackSpeed: function (value){
        value = GuitPySynth.Synthesis.SynthHelper.ClampD(value, 0.125, 8);
        this._playbackSpeed = value;
        this._synth.postMessage({
            cmd: "GuitPySynth.setPlaybackSpeed",
            value: value
        });
    },
    ...
    set_TickPosition: function (value){
        ...
        this._synth.postMessage({
            cmd: "GuitPySynth.setTickPosition",
            value: value
        });
    },
    ...
    set_TimePosition: function (value){
        ...
        this._synth.postMessage({
            cmd: "GuitPySynth.setTimePosition",
            value: value
        });
    },
    ...
    set_PlaybackRange: function (value){
        ...
        this._synth.postMessage({
            cmd: "GuitPySynth.setPlaybackRange",
            value: value
        });
    },
    Play: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.play"
        });
    },
    Pause: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.pause"
        });
    },
    PlayPause: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.playPause"
        });
    },
    Stop: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.stop"
        });
    },
    LoadSoundFont: function (data){
        if (typeof(data) == "string"){
            var url = data;
            GuitPySynth.Util.Logger.Info("Start loading Soundfont from url " + url);
            ...
            request.onload = $CreateAnonymousDelegate(this, function (e){
                ...
                this._synth.postMessage({
                    cmd: "GuitPySynth.loadSoundFontBytes",
                    data: buffer
                });
            });
            request.onerror = $CreateAnonymousDelegate(this, function (e){
                GuitPySynth.Util.Logger.Error("Loading failed: " + e.message);
                this.TriggerEvent("soundFontLoadFailed", null);
            });
            request.onprogress = $CreateAnonymousDelegate(this, function (e){
                GuitPySynth.Util.Logger.Debug("Soundfont downloading: " + e.loaded + "/" + e.total + " bytes");
    ...
    LoadMidi: function (data){
        if (typeof(data) == "string"){
            var url = data;
            GuitPySynth.Util.Logger.Info("Start loading midi from url " + url);
            var request = new XMLHttpRequest();
            request.open("GET", url, true);
            request.responseType = "arraybuffer";
            request.onload = $CreateAnonymousDelegate(this, function (e){
            ...
            request.onprogress = $CreateAnonymousDelegate(this, function (e){
                GuitPySynth.Util.Logger.Debug("Midi downloading: " + e.loaded + "/" + e.total + " bytes");
    ...
    SetChannelMute: function (channel, mute){
        this._synth.postMessage({
            cmd: "GuitPySynth.setChannelMute",
            channel: channel,
            mute: mute
        });
    },
    ResetChannelStates: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.resetChannelStates"
        });
    },
    SetChannelSolo: function (channel, solo){
        this._synth.postMessage({
            cmd: "GuitPySynth.setChannelSolo",
            channel: channel,
            solo: solo
        });
    },
    SetChannelVolume: function (channel, volume){
        volume = GuitPySynth.Synthesis.SynthHelper.ClampD(volume, 0, 10);
        this._synth.postMessage({
            cmd: "GuitPySynth.setChannelVolume",
            channel: channel,
            volume: volume
        });
    },
    SetChannelProgram: function (channel, program){
        program = GuitPySynth.Synthesis.SynthHelper.ClampB(program, 0, 127);
        this._synth.postMessage({
            cmd: "GuitPySynth.setChannelProgram",
            channel: channel,
            program: program
        });
    },
    HandleWorkerMessage: function (e){
        ...
        var cmd = data["cmd"];
        switch (cmd){
            case "GuitPySynth.ready":
                this._workerIsReady = true;
                this.CheckReady();
                break;
            case "GuitPySynth.readyForPlayback":
                this._workerIsReadyForPlayback = true;
                this.CheckReadyForPlayback();
                break;
            case "GuitPySynth.positionChanged":
                this._timePosition = data["currentTime"];
                this._tickPosition = data["currentTick"];
                this.TriggerEvent("positionChanged", [data]);
                break;
            case "GuitPySynth.playerStateChanged":
                this._state = data["state"];
                this.TriggerEvent("playerStateChanged", [data]);
                break;
            case "GuitPySynth.finished":
                this.TriggerEvent("finished", null);
                break;
            case "GuitPySynth.soundFontLoaded":
                this.TriggerEvent("soundFontLoaded", null);
                break;
            case "GuitPySynth.soundFontLoadFailed":
                this.TriggerEvent("soundFontLoadFailed", null);
                break;
            case "GuitPySynth.midiLoaded":
                this._isMidiLoaded = true;
                this.CheckReadyForPlayback();
                this.TriggerEvent("midiFileLoaded", null);
                break;
            case "GuitPySynth.midiLoadFailed":
                this._isSoundFontLoaded = true;
                this.CheckReadyForPlayback();
                this.TriggerEvent("midiFileLoadFailed", null);
                break;
            case "GuitPySynth.log":
                GuitPySynth.Util.Logger.Log(data["level"], data["message"]);
                break;
            case "GuitPySynth.output.sequencerFinished":
                this._output.SequencerFinished();
                break;
            case "GuitPySynth.output.addSamples":
                this._output.AddSamples(data["samples"]);
                break;
            case "GuitPySynth.output.play":
                this._output.Play();
                break;
            case "GuitPySynth.output.pause":
                this._output.Pause();
                break;
            case "GuitPySynth.output.resetSamples":
    ...
    OnOutputSampleRequest: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.output.sampleRequest"
        });
    },
    OnOutputFinished: function (){
        this._synth.postMessage({
            cmd: "GuitPySynth.output.finished"
        });
    },
    OnOutputSamplesPlayed: function (samples){
        this._synth.postMessage({
            cmd: "GuitPySynth.output.samplesPlayed",
            samples: samples
        });
    },
...
GuitPySynth.Platform = GuitPySynth.Platform || {};
GuitPySynth.Platform.Platform = function (){
};
GuitPySynth.Platform.Platform.CreateOutput = function (){
    return new GuitPySynth.Player.WebWorkerOutput();
};
$StaticConstructor(function (){
    GuitPySynth.Platform.Platform.ScriptFile = null;
    GuitPySynth.Platform.Platform.PlatformInit();
});
GuitPySynth.Platform.Platform.PlatformInit = function (){
    ...
    if (self.document){
        ...
        if (!scriptElement){
            ...
            try{
                ...
                GuitPySynth.Platform.Platform.ScriptFile = GuitPySynth.Platform.Platform.ScriptFileFromStack(stack);
            }
            catch(e){
                var stack = e["stack"];
                if (!stack){
                    scriptElement = document.querySelector("script[data-GuitPySynth]");
                }
                else {
                    GuitPySynth.Platform.Platform.ScriptFile = GuitPySynth.Platform.Platform.ScriptFileFromStack(stack);
                }
        ...
        if (((GuitPySynth.Platform.Platform.ScriptFile==null)||(GuitPySynth.Platform.Platform.ScriptFile.length==0))){
            if (!scriptElement){
                console.warn("Could not automatically find GuitPySynth script file for worker, please add the data-GuitPySynth attribute to the script tag that includes GuitPySynth or provide it when initializing GuitPySynth");
            }
            else {
                GuitPySynth.Platform.Platform.ScriptFile = scriptElement.src;
            }
        }
    }
};
GuitPySynth.Platform.Platform.ScriptFileFromStack = function (stack){
...
GuitPySynth.Platform.Std = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Platform.Std.IsLittleEndian = true;
});
GuitPySynth.Platform.Std.ParseInt = function (s){
    var f = parseInt(s);
    if (isNaN(f)){
        f = 0;
    }
...
GuitPySynth.Platform.Std.StringFromCharCode = function (c){
    return "";
};
GuitPySynth.Platform.Std.Random = function (){
    return Math.random();
};
GuitPySynth.Platform.Std.BlockCopy = function (src, srcOffset, dst, dstOffset, count){
};
GuitPySynth.Platform.Std.ArrayCopy = function (src, srcOffset, dst, dstOffset, count){
    for (var i = 0; i < count; i++){
        dst[dstOffset + i] = src[srcOffset + i];
    }
};
GuitPySynth.Platform.Std.Reverse = function (array){
    var i = 0;
...
GuitPySynth.Platform.TypeUtils = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Platform.TypeUtils.IsLittleEndian = true;
    GuitPySynth.Platform.TypeUtils.Int16 = new Int16Array(1);
});
GuitPySynth.Platform.TypeUtils.ToUInt32 = function (i){
    return i | 0;
};
GuitPySynth.Platform.TypeUtils.ToInt16 = function (i){
    GuitPySynth.Platform.TypeUtils.Int16[0] = i | 0;
    return GuitPySynth.Platform.TypeUtils.Int16[0];
};
GuitPySynth.Platform.TypeUtils.ToUInt16 = function (i){
    return i | 0;
};
GuitPySynth.Platform.TypeUtils.ToUInt8 = function (i){
    return i;
};
GuitPySynth.Platform.TypeUtils.ClearIntArray = function (array){
    for (var i = 0; i < array.length; i++){
        array[i] = 0;
    }
};
GuitPySynth.Platform.TypeUtils.ClearShortArray = function (array){
    for (var i = 0; i < array.length; i++){
        array[i] = 0;
    }
};
GuitPySynth.Player = GuitPySynth.Player || {};
GuitPySynth.Player.WebWorkerOutput = function (){
    this._worker = null;
    this.Ready = null;
    this.SamplesPlayed = null;
    this.SampleRequest = null;
...
GuitPySynth.Player.WebWorkerOutput.prototype = {
    get_SampleRate: function (){
        return GuitPySynth.Player.WebWorkerOutput.PreferredSampleRate;
    },
    Open: function (){
        GuitPySynth.Util.Logger.Debug("Initializing webworker worker");
        this._worker =  self;
    ...
    HandleMessage: function (e){
        ...
        var cmd = data["cmd"];
        switch (cmd){
            case "GuitPySynth.output.sampleRequest":
                this.SampleRequest();
                break;
            case "GuitPySynth.output.finished":
                this.Finished();
                break;
            case "GuitPySynth.output.samplesPlayed":
    ...
    SequencerFinished: function (){
        this._worker.postMessage({
            cmd: "GuitPySynth.output.sequencerFinished"
        });
    },
    AddSamples: function (samples){
        this._worker.postMessage({
            cmd: "GuitPySynth.output.addSamples",
            samples: samples
        });
    },
    Play: function (){
        this._worker.postMessage({
            cmd: "GuitPySynth.output.play"
        });
    },
    Pause: function (){
        this._worker.postMessage({
            cmd: "GuitPySynth.output.pause"
        });
    },
    ResetSamples: function (){
        this._worker.postMessage({
            cmd: "GuitPySynth.output.resetSamples"
        });
    }
};
$StaticConstructor(function (){
    GuitPySynth.Player.WebWorkerOutput.CmdOutputPrefix = "GuitPySynth.output.";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputSequencerFinished = "GuitPySynth.output.sequencerFinished";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputAddSamples = "GuitPySynth.output.addSamples";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputPlay = "GuitPySynth.output.play";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputPause = "GuitPySynth.output.pause";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputResetSamples = "GuitPySynth.output.resetSamples";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputSampleRequest = "GuitPySynth.output.sampleRequest";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputFinished = "GuitPySynth.output.finished";
    GuitPySynth.Player.WebWorkerOutput.CmdOutputSamplesPlayed = "GuitPySynth.output.samplesPlayed";
    GuitPySynth.Player.WebWorkerOutput.PreferredSampleRate = 0;
});
GuitPySynth.Util = GuitPySynth.Util || {};
GuitPySynth.Util.Extensions = function (){
};
GuitPySynth.Util.Logger = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Util.Logger.LogLevel = GuitPySynth.Util.LogLevel.None;
    GuitPySynth.Util.Logger.LogHandler = null;
    GuitPySynth.Util.Logger.LogLevel = GuitPySynth.Util.LogLevel.Info;
    GuitPySynth.Util.Logger.LogHandler = function (level, message){
        switch (level){
            case GuitPySynth.Util.LogLevel.None:
                console.log(message);
                break;
            case GuitPySynth.Util.LogLevel.Debug:
                console.debug(message);
                break;
            case GuitPySynth.Util.LogLevel.Info:
                console.info(message);
                break;
            case GuitPySynth.Util.LogLevel.Warning:
                console.warn(message);
                break;
            case GuitPySynth.Util.LogLevel.Error:
                console.error(message);
                break;
        }
    };
});
GuitPySynth.Util.Logger.Debug = function (msg){
    GuitPySynth.Util.Logger.Log(GuitPySynth.Util.LogLevel.Debug, msg);
};
GuitPySynth.Util.Logger.Warning = function (msg){
    GuitPySynth.Util.Logger.Log(GuitPySynth.Util.LogLevel.Warning, msg);
};
GuitPySynth.Util.Logger.Info = function (msg){
    GuitPySynth.Util.Logger.Log(GuitPySynth.Util.LogLevel.Info, msg);
};
GuitPySynth.Util.Logger.Error = function (msg){
    GuitPySynth.Util.Logger.Log(GuitPySynth.Util.LogLevel.Error, msg);
};
GuitPySynth.Util.Logger.Log = function (logLevel, msg){
    if (logLevel < GuitPySynth.Util.Logger.LogLevel)
        return;
    ...
    if (GuitPySynth.Util.Logger.LogHandler != null){
        GuitPySynth.Util.Logger.LogHandler(logLevel, caller + "-" + msg);
    }
};
GuitPySynth.Util.Logger.GetCaller = function (){
    return "";
};
GuitPySynth.Util.LogLevel = {
    None: 0,
    Debug: 1,
    Info: 2,
    Warning: 3,
...
GuitPySynth.GuitPySynth = function (){
    ...
    this.Output = null;
    GuitPySynth.Util.Logger.Debug("Initializing player");
    this._state = GuitPySynth.PlayerState.Paused;
    GuitPySynth.Util.Logger.Debug("Creating output");
    this.Output = GuitPySynth.Platform.Platform.CreateOutput();
    ...
    this.Output.add_Finished($CreateAnonymousDelegate(this, function (){
        // stop everything
        this.Stop();
        GuitPySynth.Util.Logger.Debug("Finished playback");
    ...
    GuitPySynth.Util.Logger.Debug("Creating synthesizer");
    this._synthesizer = new GuitPySynth.Synthesis.Synthesizer(this.Output.get_SampleRate(), 2, 441, 3, 100);
    this._sequencer = new GuitPySynth.MidiFileSequencer(this._synthesizer);
    this._sequencer.add_Finished($CreateDelegate(this.Output, this.Output.SequencerFinished));
    GuitPySynth.Util.Logger.Debug("Opening output");
    this.Output.Open();
};
GuitPySynth.GuitPySynth.prototype = {
    get_IsReady: function (){
    ...
    set_TimePosition: function (value){
        GuitPySynth.Util.Logger.Debug("Seeking to position " + value + "ms");
    ...
    Play: function (){
        if (this.get_State() == GuitPySynth.PlayerState.Playing || !this.get_IsReadyForPlayback())
            return;
        GuitPySynth.Util.Logger.Debug("Starting playback");
        this._state = GuitPySynth.PlayerState.Playing;
        this.OnPlayerStateChanged(new GuitPySynth.PlayerStateChangedEventArgs(this._state));
        this.Output.Play();
    },
    Pause: function (){
        if (this.get_State() == GuitPySynth.PlayerState.Paused || !this.get_IsReadyForPlayback())
            return;
        GuitPySynth.Util.Logger.Debug("Pausing playback");
        this._state = GuitPySynth.PlayerState.Paused;
        this.OnPlayerStateChanged(new GuitPySynth.PlayerStateChangedEventArgs(this._state));
        this.Output.Pause();
    ...
    Stop: function (){
        if (!this.get_IsReadyForPlayback())
            return;
        GuitPySynth.Util.Logger.Debug("Stopping playback");
        this.Pause();
        this.set_TickPosition(this._sequencer.get_PlaybackRange() != null ? this._sequencer.get_PlaybackRange().StartTick : 0);
    },
    LoadSoundFont: function (data){
        this.Pause();
        var input = GuitPySynth.IO.ByteBuffer.FromBuffer(data);
        try{
            GuitPySynth.Util.Logger.Info("Loading soundfont from bytes");
            var bank = new GuitPySynth.Bank.PatchBank();
            bank.LoadSf2(input);
            this._synthesizer.LoadBank(bank);
            this._isSoundFontLoaded = true;
            this.OnSoundFontLoaded();
            GuitPySynth.Util.Logger.Info("soundFont successfully loaded");
            this.CheckReadyForPlayback();
        }
        catch(e){
            GuitPySynth.Util.Logger.Error("Could not load soundfont from bytes " + e);
    ...
    LoadMidi: function (data){
        ...
        var input = GuitPySynth.IO.ByteBuffer.FromBuffer(data);
        try{
            GuitPySynth.Util.Logger.Info("Loading midi from bytes");
            var midi = new GuitPySynth.Midi.MidiFile();
            midi.Load(input);
            this._sequencer.LoadMidi(midi);
            this._isMidiLoaded = true;
            this.OnMidiLoaded();
            GuitPySynth.Util.Logger.Info("Midi successfully loaded");
            this.CheckReadyForPlayback();
            this.set_TickPosition(0);
        }
        catch(e){
            GuitPySynth.Util.Logger.Error("Could not load midi from bytes " + e);
    ...
    UpdateTimePosition: function (timePosition){
        ...
        GuitPySynth.Util.Logger.Debug("Position changed: (time: " + currentTime + "/" + endTime + ", tick: " + currentTick + "/" + endTime + ")");
        this.OnPositionChanged(new GuitPySynth.PositionChangedEventArgs(currentTime, endTime, currentTick, endTick));
    },
    add_Finished: function (value){
        this.Finished = $CombineDelegates(this.Finished, value);
    },
    ...
    OnSoundFontLoaded: function (){
        ...
        if (handler != null)
            handler(this, GuitPySynth.EmptyEventArgs.Instance);
    },
...
GuitPySynth.EmptyEventArgs = function (){
};
$StaticConstructor(function (){
    GuitPySynth.EmptyEventArgs.Instance = new GuitPySynth.EmptyEventArgs();
});
GuitPySynth.ProgressEventArgs = function (loaded, total){
    this.Loaded = 0;
    this.Total = 0;
    this.Loaded = loaded;
    this.Total = total;
};
GuitPySynth.PlayerStateChangedEventArgs = function (state){
    this.State = GuitPySynth.PlayerState.Paused;
    this.State = state;
};
GuitPySynth.PositionChangedEventArgs = function (currentTime, endTime, currentTick, endTick){
    this.CurrentTime = 0;
    this.EndTime = 0;
    this.CurrentTick = 0;
    this.EndTick = 0;
...
GuitPySynth.Bank = GuitPySynth.Bank || {};
GuitPySynth.Bank.AssetManager = function (){
    this.PatchAssets = null;
    this.SampleAssets = null;
    this.PatchAssets = [];
    this.SampleAssets = [];
};
GuitPySynth.Bank.AssetManager.prototype = {
...
GuitPySynth.Bank.Components = GuitPySynth.Bank.Components || {};
GuitPySynth.Bank.Components.Generators = GuitPySynth.Bank.Components.Generators || {};
GuitPySynth.Bank.Components.Generators.Generator = function (description){
    this.LoopMode = GuitPySynth.Bank.Components.Generators.LoopMode.NoLoop;
...
GuitPySynth.Bank.Components.Generators.Generator.prototype = {
    Release: function (generatorParams){
        if (this.LoopMode == GuitPySynth.Bank.Components.Generators.LoopMode.LoopUntilNoteOff){
            generatorParams.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.PostLoop;
        ...
        do{
            ...
            if (samplesAvailable > blockBuffer.length - proccessed){
                ...
                switch (generatorParams.CurrentState){
                    case GuitPySynth.Bank.Components.Generators.GeneratorState.PreLoop:
                        generatorParams.CurrentStart = this.LoopStartPhase;
                        generatorParams.CurrentEnd = this.LoopEndPhase;
                        generatorParams.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.Loop;
                        break;
                    case GuitPySynth.Bank.Components.Generators.GeneratorState.Loop:
                        generatorParams.Phase += generatorParams.CurrentStart - generatorParams.CurrentEnd;
                        break;
                    case GuitPySynth.Bank.Components.Generators.GeneratorState.PostLoop:
                        generatorParams.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.Finished;
                        while (proccessed < blockBuffer.length)
...
GuitPySynth.Bank.Components.Generators.WhiteNoiseGenerator = function (description){
    GuitPySynth.Bank.Components.Generators.Generator.call(this, description);
    if (this.EndPhase < 0)
        this.EndPhase = 1;
...
GuitPySynth.Bank.Components.Generators.WhiteNoiseGenerator.prototype = {
    GetValue: function (phase){
        return ((GuitPySynth.Platform.Std.Random() * 2) - 1);
    }
};
$Inherit(GuitPySynth.Bank.Components.Generators.WhiteNoiseGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.TriangleGenerator = function (description){
    GuitPySynth.Bank.Components.Generators.Generator.call(this, description);
    if (this.EndPhase < 0)
        this.EndPhase = 1.25;
    if (this.StartPhase < 0)
...
GuitPySynth.Bank.Components.Generators.TriangleGenerator.prototype = {
    GetValue: function (phase){
        return (Math.abs(phase - Math.Floor(phase + 0.5)) * 4 - 1);
    }
};
$Inherit(GuitPySynth.Bank.Components.Generators.TriangleGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.SquareGenerator = function (description){
    GuitPySynth.Bank.Components.Generators.Generator.call(this, description);
    if (this.EndPhase < 0)
...
GuitPySynth.Bank.Components.Generators.SquareGenerator.prototype = {
    GetValue: function (phase){
        return Math.Sign(Math.sin(phase));
    }
};
$Inherit(GuitPySynth.Bank.Components.Generators.SquareGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.SineGenerator = function (description){
    GuitPySynth.Bank.Components.Generators.Generator.call(this, description);
    if (this.EndPhase < 0)
        this.EndPhase = 6.28318530717958;
    if (this.StartPhase < 0)
...
GuitPySynth.Bank.Components.Generators.SineGenerator.prototype = {
    GetValue: function (phase){
        return Math.sin(phase);
    }
};
$Inherit(GuitPySynth.Bank.Components.Generators.SineGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.SawGenerator = function (description){
    GuitPySynth.Bank.Components.Generators.Generator.call(this, description);
    if (this.EndPhase < 0)
        this.EndPhase = 1;
    if (this.StartPhase < 0)
...
GuitPySynth.Bank.Components.Generators.SawGenerator.prototype = {
    GetValue: function (phase){
        return (2 * (phase - Math.Floor(phase + 0.5)));
    }
};
$Inherit(GuitPySynth.Bank.Components.Generators.SawGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.Interpolation = {
    None: 0,
    Linear: 1,
    Cosine: 2,
    CubicSpline: 3,
...
GuitPySynth.Bank.Components.Generators.SampleGenerator = function (){
    this.Samples = null;
    GuitPySynth.Bank.Components.Generators.Generator.call(this, new GuitPySynth.Bank.Descriptors.GeneratorDescriptor());
};
GuitPySynth.Bank.Components.Generators.SampleGenerator.prototype = {
    GetValue: function (phase){
    ...
    Interpolate: function (generatorParams, blockBuffer, increment, start, end){
        switch (1){
            case GuitPySynth.Bank.Components.Generators.Interpolation.Linear:
                {
                var _end = generatorParams.CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Loop ? this.LoopEndPhase - 1 : this.EndPhase - 1;
                var index;
                var s1,s2,mu;
                ...
                while (start < end){
                    index = generatorParams.Phase | 0;
                    s1 = this.Samples.get_Item(index);
                    if (generatorParams.CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Loop)
                ...
                break;
            case GuitPySynth.Bank.Components.Generators.Interpolation.Cosine:
                {
                var _end = generatorParams.CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Loop ? this.LoopEndPhase - 1 : this.EndPhase - 1;
                var index;
                var s1,s2,mu;
                ...
                break;
            case GuitPySynth.Bank.Components.Generators.Interpolation.CubicSpline:
                {
                var _end = generatorParams.CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Loop ? this.LoopStartPhase + 1 : this.StartPhase + 1;
                var index;
                var s0,s1,s2,s3,mu;
                while (start < end && generatorParams.Phase < _end){
                    index = generatorParams.Phase | 0;
                ...
                _end = generatorParams.CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Loop ? this.LoopEndPhase - 2 : this.EndPhase - 2;
                while (start < end && generatorParams.Phase < _end){
                    index = generatorParams.Phase | 0;
...
$Inherit(GuitPySynth.Bank.Components.Generators.SampleGenerator, GuitPySynth.Bank.Components.Generators.Generator);
GuitPySynth.Bank.Components.Generators.GeneratorParameters = function (){
    this.Phase = 0;
    this.CurrentStart = 0;
    this.CurrentEnd = 0;
    this.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.PreLoop;
    this.Phase = 0;
    this.CurrentStart = 0;
    this.CurrentEnd = 0;
    this.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.PreLoop;
};
GuitPySynth.Bank.Components.Generators.GeneratorParameters.prototype = {
    QuickSetup: function (generator){
        ...
        switch (generator.LoopMode){
            case GuitPySynth.Bank.Components.Generators.LoopMode.Continuous:
            case GuitPySynth.Bank.Components.Generators.LoopMode.LoopUntilNoteOff:
                if (this.Phase >= generator.EndPhase){
                //phase is greater than the end index so generator is finished
                this.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.Finished;
            }
                else if (this.Phase >= generator.LoopEndPhase){
                //phase is greater than the loop end point so generator is in post loop
                this.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.PostLoop;
                ...
                this.CurrentState = GuitPySynth.Bank.Components.Generators.GeneratorState.Loop;
...
GuitPySynth.Bank.Components.Generators.LoopMode = {
    NoLoop: 0,
    OneShot: 1,
    Continuous: 2,
    LoopUntilNoteOff: 3
};
GuitPySynth.Bank.Components.Generators.GeneratorState = {
    PreLoop: 0,
    Loop: 1,
    PostLoop: 2,
    Finished: 3
};
GuitPySynth.Bank.Components.Generators.DefaultGenerators = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSine = new GuitPySynth.Bank.Components.Generators.SineGenerator(new GuitPySynth.Bank.Descriptors.GeneratorDescriptor());
    GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSaw = new GuitPySynth.Bank.Components.Generators.SawGenerator(new GuitPySynth.Bank.Descriptors.GeneratorDescriptor());
    GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSquare = new GuitPySynth.Bank.Components.Generators.SquareGenerator(new GuitPySynth.Bank.Descriptors.GeneratorDescriptor());
    GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultTriangle = new GuitPySynth.Bank.Components.Generators.TriangleGenerator(new GuitPySynth.Bank.Descriptors.GeneratorDescriptor());
});
GuitPySynth.Bank.Components.LfoState = {
    Delay: 0,
    Sustain: 1
};
GuitPySynth.Bank.Components.Lfo = function (){
    this._phase = 0;
    this._increment = 0;
    this._delayTime = 0;
    this._generator = null;
    this.Frequency = 0;
    this.CurrentState = GuitPySynth.Bank.Components.LfoState.Delay;
    this.Value = 0;
    this.Depth = 0;
    this.CurrentState = GuitPySynth.Bank.Components.LfoState.Delay;
    this._generator = GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSine;
    this._delayTime = 0;
    this._increment = 0;
    this._phase = 0;
    this.Frequency = 0;
...
GuitPySynth.Bank.Components.Lfo.prototype = {
    ...
    Increment: function (amount){
        if (this.CurrentState == GuitPySynth.Bank.Components.LfoState.Delay){
            this._phase -= amount;
            if (this._phase <= 0){
                this._phase = this._generator.LoopStartPhase + this._increment * -this._phase;
                this.Value = this._generator.GetValue(this._phase);
                this.CurrentState = GuitPySynth.Bank.Components.LfoState.Sustain;
            }
        }
        else {
...
GuitPySynth.Bank.Components.FilterType = {
    None: 0,
    BiquadLowpass: 1,
    BiquadHighpass: 2,
    OnePoleLowpass: 3
};
GuitPySynth.Bank.Components.Filter = function (){
    this._a1 = 0;
    this._a2 = 0;
    this._b1 = 0;
    this._b2 = 0;
    ...
    this._cutOff = 0;
    this._resonance = 0;
    this.FilterMethod = GuitPySynth.Bank.Components.FilterType.None;
...
GuitPySynth.Bank.Components.Filter.prototype = {
    get_CutOff: function (){
        return this._cutOff;
    },
    ...
    get_Enabled: function (){
        return this.FilterMethod != GuitPySynth.Bank.Components.FilterType.None;
    },
    Disable: function (){
        this.FilterMethod = GuitPySynth.Bank.Components.FilterType.None;
    },
    QuickSetup: function (sampleRate, note, velocity, filterInfo){
        ...
        if (this.FilterMethod != GuitPySynth.Bank.Components.FilterType.None){
            this.set_CutOff(this.get_CutOff() * GuitPySynth.Synthesis.SynthHelper.CentsToPitch((note - filterInfo.RootKey) * filterInfo.KeyTrack + ((velocity * filterInfo.VelTrack)) | 0));
            this.UpdateCoefficients(sampleRate);
        }
    },
    ApplyFilter: function (sample){
        switch (this.FilterMethod){
            case GuitPySynth.Bank.Components.FilterType.BiquadHighpass:
            case GuitPySynth.Bank.Components.FilterType.BiquadLowpass:
                this._m3 = sample - this._a1 * this._m1 - this._a2 * this._m2;
                sample = this._b2 * (this._m3 + this._m2) + this._b1 * this._m1;
                this._m2 = this._m1;
                this._m1 = this._m3;
                return sample;
            case GuitPySynth.Bank.Components.FilterType.OnePoleLowpass:
    ...
    GenerateFilterCoeff: function (fc, q){
        fc = GuitPySynth.Synthesis.SynthHelper.ClampD(fc, 1E-38, 0.49);
...
GuitPySynth.Bank.Components.EnvelopeState = {
    Delay: 0,
    Attack: 1,
    Hold: 2,
    Decay: 3,
...
GuitPySynth.Bank.Components.Envelope = function (){
    this._stages = null;
    this._index = 0;
    this._stage = null;
    this.Value = 0;
    this.CurrentStage = GuitPySynth.Bank.Components.EnvelopeState.Delay;
    this.Depth = 0;
    this.Value = 0;
    this.Depth = 0;
    this._stages = new Array(7);
    for (var x = 0; x < this._stages.length; x++){
        this._stages[x] = new GuitPySynth.Bank.Components.EnvelopeStage();
        this._stages[x].Graph = GuitPySynth.Util.Tables.EnvelopeTables(0);
    }
...
GuitPySynth.Bank.Components.Envelope.prototype = {
    QuickSetupSf2: function (sampleRate, note, keyNumToHold, keyNumToDecay, isVolumeEnvelope, envelopeInfo){
        ...
        this._stages[1].Graph = GuitPySynth.Util.Tables.EnvelopeTables(envelopeInfo.AttackGraph);
        // Hold
        ...
        this._stages[3].Graph = GuitPySynth.Util.Tables.EnvelopeTables(envelopeInfo.DecayGraph);
        // Sustain
        ...
        this._stages[5].Graph = GuitPySynth.Util.Tables.EnvelopeTables(envelopeInfo.ReleaseGraph);
        this._index = 0;
        this.Value = 0;
        this.CurrentStage = GuitPySynth.Bank.Components.EnvelopeState.Delay;
    ...
    Increment: function (samples){
        do{
            ...
            if (neededSamples > samples){
                ...
                this._index = 0;
                if (this.CurrentStage != GuitPySynth.Bank.Components.EnvelopeState.None){
                    do{
                        this._stage = this._stages[++this.CurrentStage];
                    }
    ...
    Release: function (lowerLimit){
        if (this.Value <= lowerLimit){
            this._index = 0;
            this.CurrentStage = GuitPySynth.Bank.Components.EnvelopeState.None;
            this._stage = this._stages[this.CurrentStage];
        }
        else if (this.CurrentStage < GuitPySynth.Bank.Components.EnvelopeState.Release){
            this._index = 0;
            this.CurrentStage = GuitPySynth.Bank.Components.EnvelopeState.Release;
            this._stage = this._stages[this.CurrentStage];
...
GuitPySynth.Bank.Components.EnvelopeStage = function (){
    this.Time = 0;
    this.Graph = null;
    this.Scale = 0;
    this.Offset = 0;
...
GuitPySynth.Bank.Components.PanFormulaEnum = {
    Neg3dBCenter: 0,
    Neg6dBCenter: 1,
    ZeroCenter: 2
};
GuitPySynth.Bank.Components.PanComponent = function (){
    this.Left = 0;
    this.Right = 0;
};
GuitPySynth.Bank.Components.PanComponent.prototype = {
    SetValue: function (value, formula){
        value = GuitPySynth.Synthesis.SynthHelper.ClampF(value, -1, 1);
        var dvalue;
        switch (formula){
            case GuitPySynth.Bank.Components.PanFormulaEnum.Neg3dBCenter:
                dvalue = 1.5707963267949 * (value + 1) / 2;
                this.Left = Math.cos(dvalue);
                this.Right = Math.sin(dvalue);
                break;
            case GuitPySynth.Bank.Components.PanFormulaEnum.Neg6dBCenter:
                this.Left = (0.5 + value * -0.5);
                this.Right = (0.5 + value * 0.5);
                break;
            case GuitPySynth.Bank.Components.PanFormulaEnum.ZeroCenter:
                dvalue = 1.5707963267949 * (value + 1) / 2;
...
GuitPySynth.Bank.Descriptors = GuitPySynth.Bank.Descriptors || {};
GuitPySynth.Bank.Descriptors.LfoDescriptor = function (){
    this.DelayTime = 0;
    this.Frequency = 0;
    this.Depth = 0;
    this.Generator = null;
    ...
    this.Frequency = 8;
    this.Depth = 1;
    this.Generator = GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSine;
};
GuitPySynth.Bank.Descriptors.Waveform = {
    Sine: 0,
    Square: 1,
    Saw: 2,
    Triangle: 3,
...
GuitPySynth.Bank.Descriptors.GeneratorDescriptor = function (){
    this.LoopMethod = GuitPySynth.Bank.Components.Generators.LoopMode.NoLoop;
    this.SamplerType = GuitPySynth.Bank.Descriptors.Waveform.Sine;
    this.AssetName = null;
    this.EndPhase = 0;
    this.StartPhase = 0;
    this.LoopEndPhase = 0;
...
GuitPySynth.Bank.Descriptors.FilterDescriptor = function (){
    this.FilterMethod = GuitPySynth.Bank.Components.FilterType.None;
    this.CutOff = 0;
    this.Resonance = 0;
    this.RootKey = 0;
...
GuitPySynth.Bank.Descriptors.EnvelopeDescriptor = function (){
    this.DelayTime = 0;
    this.AttackTime = 0;
    this.AttackGraph = 0;
    this.HoldTime = 0;
...
GuitPySynth.Bank.Patch = GuitPySynth.Bank.Patch || {};
GuitPySynth.Bank.Patch.Patch = function (name){
...
GuitPySynth.Bank.Patch.Sf2Patch = function (name){
    this.iniFilterFc = 0;
    this.filterQ = 0;
    this.initialAttn = 0;
    this.keyOverride = 0;
    ...
    this.vib_lfo = null;
    this.fltr = null;
    GuitPySynth.Bank.Patch.Patch.call(this, name);
};
GuitPySynth.Bank.Patch.Sf2Patch.prototype = {
    Start: function (voiceparams){
        ...
        voiceparams.VolOffset -= 96 * GuitPySynth.Bank.Patch.Sf2Patch.CalculateModulator(GuitPySynth.Sf2.SourceTypeEnum.Concave, GuitPySynth.Sf2.TransformEnum.Linear, GuitPySynth.Sf2.DirectionEnum.MaxToMin, GuitPySynth.Sf2.PolarityEnum.Unipolar, voiceparams.Velocity, 0, 127);
        voiceparams.VolOffset -= 96 * GuitPySynth.Bank.Patch.Sf2Patch.CalculateModulator(GuitPySynth.Sf2.SourceTypeEnum.Concave, GuitPySynth.Sf2.TransformEnum.Linear, GuitPySynth.Sf2.DirectionEnum.MaxToMin, GuitPySynth.Sf2.PolarityEnum.Unipolar, voiceparams.SynthParams.Volume.get_Coarse(), 0, 127);
        //check if we have finished before we have begun
        return voiceparams.GeneratorParams[0].CurrentState != GuitPySynth.Bank.Components.Generators.GeneratorState.Finished && voiceparams.Envelopes[1].CurrentStage != GuitPySynth.Bank.Components.EnvelopeState.None;
    },
    Stop: function (voiceparams){
        this.gen.Release(voiceparams.GeneratorParams[0]);
        if (this.gen.LoopMode != GuitPySynth.Bank.Components.Generators.LoopMode.OneShot){
    ...
    Process: function (voiceparams, startIndex, endIndex, isMuted){
        ...
        var basePitchFrequency = GuitPySynth.Synthesis.SynthHelper.CentsToPitch(voiceparams.SynthParams.CurrentPitch) * this.gen.Frequency;
        var pitchWithBend = basePitchFrequency * GuitPySynth.Synthesis.SynthHelper.CentsToPitch(voiceparams.PitchOffset);
        ...
        for (var x = startIndex; x < endIndex; x += 128){
            ...
            this.gen.GetValues(voiceparams.GeneratorParams[0], voiceparams.BlockBuffer, basePitch * GuitPySynth.Synthesis.SynthHelper.CentsToPitch(((voiceparams.Envelopes[0].Value * this.modEnvToPitch + voiceparams.Lfos[0].Value * this.modLfoToPitch + voiceparams.Lfos[1].Value * this.vibLfoToPitch)) | 0));
            //--Filter
            if (voiceparams.Filters[0].get_Enabled()){
                ...
                if (centsFc > 13500)
                    centsFc = 13500;
                voiceparams.Filters[0].set_CutOff(GuitPySynth.Synthesis.SynthHelper.KeyToFrequency(centsFc / 100, 69));
            ...
            //--Volume calculation
            var volume = GuitPySynth.Synthesis.SynthHelper.DBtoLinear(voiceparams.VolOffset + voiceparams.Envelopes[1].Value + voiceparams.Lfos[0].Value * this.modLfoToVolume) * baseVolume;
            ...
            if ((voiceparams.Envelopes[1].CurrentStage > GuitPySynth.Bank.Components.EnvelopeState.Hold && volume <= 1E-05) || voiceparams.GeneratorParams[0].CurrentState == GuitPySynth.Bank.Components.Generators.GeneratorState.Finished){
                voiceparams.State = GuitPySynth.Synthesis.VoiceStateEnum.Stopped;
                return;
            }
        }
    },
    Load: function (region, assets){
        ...
        this.filterQ = GuitPySynth.Synthesis.SynthHelper.DBtoLinear(region.Generators[9] / 10);
        ...
        this.pan = new GuitPySynth.Bank.Components.PanComponent();
        this.pan.SetValue(region.Generators[17] / 500, GuitPySynth.Bank.Components.PanFormulaEnum.Neg3dBCenter);
    ...
    LoadGen: function (region, assets){
        ...
        this.gen = new GuitPySynth.Bank.Components.Generators.SampleGenerator();
        ...
        switch (region.Generators[54] & 3){
            case 0:
            case 2:
                this.gen.LoopMode = GuitPySynth.Bank.Components.Generators.LoopMode.NoLoop;
                break;
            case 1:
                this.gen.LoopMode = GuitPySynth.Bank.Components.Generators.LoopMode.Continuous;
                break;
            case 3:
                this.gen.LoopMode = GuitPySynth.Bank.Components.Generators.LoopMode.LoopUntilNoteOff;
                break;
        }
    ...
    LoadEnvelopes: function (region){
        //
        //mod env
        this.mod_env = new GuitPySynth.Bank.Descriptors.EnvelopeDescriptor();
        ...
        this.mod_env.SustainLevel = 1 - GuitPySynth.Synthesis.SynthHelper.ClampS(region.Generators[29], 0, 1000) / 1000;
        //checks
        ...
        //
        // volume env
        this.vel_env = new GuitPySynth.Bank.Descriptors.EnvelopeDescriptor();
        ...
        this.vel_env.SustainLevel = GuitPySynth.Synthesis.SynthHelper.ClampS(region.Generators[37], 0, 1000) / -10;
        // checks
    ...
    LoadLfos: function (region){
        this.mod_lfo = new GuitPySynth.Bank.Descriptors.LfoDescriptor();
        ...
        this.mod_lfo.Generator = GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSine;
        this.vib_lfo = new GuitPySynth.Bank.Descriptors.LfoDescriptor();
        ...
        this.vib_lfo.Generator = GuitPySynth.Bank.Components.Generators.DefaultGenerators.DefaultSine;
    },
    LoadFilter: function (region){
        this.fltr = new GuitPySynth.Bank.Descriptors.FilterDescriptor();
        this.fltr.FilterMethod = GuitPySynth.Bank.Components.FilterType.BiquadLowpass;
        this.fltr.CutOff = GuitPySynth.Synthesis.SynthHelper.KeyToFrequency(region.Generators[8] / 100, 69);
        this.fltr.Resonance = GuitPySynth.Synthesis.SynthHelper.DBtoLinear(region.Generators[9] / 10);
    }
};
GuitPySynth.Bank.Patch.Sf2Patch.CalculateModulator = function (s, t, d, p, value, min, max){
    var output = 0;
    var i;
    value = value - min;
    max = max - min;
    if (d == GuitPySynth.Sf2.DirectionEnum.MaxToMin)
        value = max - value;
    switch (s){
        case GuitPySynth.Sf2.SourceTypeEnum.Linear:
            output = (value / max) | 0;
            break;
        case GuitPySynth.Sf2.SourceTypeEnum.Concave:
            i = 127 - value;
            output = -0.208333333333333 * (Math.log((i * i) / (max * max))/Math.LN10);
            break;
        case GuitPySynth.Sf2.SourceTypeEnum.Convex:
            i = value;
            output = 1 + (0.208333333333333) * (Math.log((i * i) / (max * max))/Math.LN10);
            break;
        case GuitPySynth.Sf2.SourceTypeEnum.Switch:
            if (value <= ((max / 2) | 0))
            output = 0;
            else
            output = 1;
    ...
    if (p == GuitPySynth.Sf2.PolarityEnum.Bipolar)
        output = (output * 2) - 1;
    if (t == GuitPySynth.Sf2.TransformEnum.AbsoluteValue)
        output = Math.abs(output);
    return output;
};
$Inherit(GuitPySynth.Bank.Patch.Sf2Patch, GuitPySynth.Bank.Patch.Patch);
GuitPySynth.Bank.Patch.MultiPatch = function (name){
    this._intervalType = GuitPySynth.Bank.Patch.IntervalType.ChannelKeyVelocity;
    this._intervalList = null;
    GuitPySynth.Bank.Patch.Patch.call(this, name);
    this._intervalType = GuitPySynth.Bank.Patch.IntervalType.ChannelKeyVelocity;
};
GuitPySynth.Bank.Patch.MultiPatch.prototype = {
    FindPatches: function (channel, key, velocity, layers){
        var count = 0;
        switch (this._intervalType){
            case GuitPySynth.Bank.Patch.IntervalType.ChannelKeyVelocity:
                ...
                break;
            case GuitPySynth.Bank.Patch.IntervalType.ChannelKey:
                for (var x = 0; x < this._intervalList.length; x++){
                ...
                break;
            case GuitPySynth.Bank.Patch.IntervalType.KeyVelocity:
                for (var x = 0; x < this._intervalList.length; x++){
                ...
                break;
            case GuitPySynth.Bank.Patch.IntervalType.Key:
                for (var x = 0; x < this._intervalList.length; x++){
    ...
    LoadSf2: function (regions, assets){
        ...
        for (var x = 0; x < regions.length; x++){
            ...
            var loVel;
            var hiVel;
            if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
            ...
            var sf2 = new GuitPySynth.Bank.Patch.Sf2Patch(this.Name + "_" + x);
            sf2.Load(regions[x], assets);
            this._intervalList[x] = new GuitPySynth.Bank.Patch.PatchInterval(sf2, 0, 15, loKey, hiKey, loVel, hiVel);
        }
    ...
    DetermineIntervalType: function (){
        ...
        if (checkChannel & checkVelocity)
            ...
            this._intervalType = GuitPySynth.Bank.Patch.IntervalType.ChannelKey;
        else if (checkVelocity)
            this._intervalType = GuitPySynth.Bank.Patch.IntervalType.KeyVelocity;
        else
            this._intervalType = GuitPySynth.Bank.Patch.IntervalType.Key;
    }
};
$Inherit(GuitPySynth.Bank.Patch.MultiPatch, GuitPySynth.Bank.Patch.Patch);
GuitPySynth.Bank.Patch.IntervalType = {
    ChannelKeyVelocity: 0,
    ChannelKey: 1,
    KeyVelocity: 2,
    Key: 3
};
GuitPySynth.Bank.Patch.PatchInterval = function (patch, startChannel, endChannel, startKey, endKey, startVelocity, endVelocity){
    this.Patch = null;
    this.StartChannel = 0;
    this.StartKey = 0;
...
GuitPySynth.Bank.Patch.PatchInterval.prototype = {
...
GuitPySynth.Bank.SampleDataAsset = function (sample, sampleData){
    this.Name = null;
    this.Channels = 0;
    this.SampleRate = 0;
    this.RootKey = 0;
    ...
    if ((sample.SoundFontSampleLink & GuitPySynth.Sf2.SFSampleLink.OggVobis) != 0){
        throw $CreateException(new System.Exception.ctor$$String("Ogg Vobis encoded soundfonts not supported"), new Error());
    }
    else {
        this.SampleData = GuitPySynth.Util.PcmData.Create(sampleData.BitsPerSample, sampleData.SampleData, true);
    }
};
GuitPySynth.Bank.PatchBank = function (){
    this._bank = null;
    this._assets = null;
    this.Name = null;
    this.Comments = null;
...
GuitPySynth.Bank.PatchBank.prototype = {
    Reset: function (){
        this._bank = {};
        this._assets = new GuitPySynth.Bank.AssetManager();
        this.Name = "";
        this.Comments = "";
    },
    get_LoadedBanks: function (){
        ...
        for (var $i2 = 0,$t2 = Object.keys(this._bank),$l2 = $t2.length,bank = $t2[$i2]; $i2 < $l2; $i2++, bank = $t2[$i2]){
            banks.push(GuitPySynth.Platform.Std.ParseInt(bank));
        }
    ...
    LoadSf2: function (input){
        this.Reset();
        GuitPySynth.Util.Logger.Debug("Reading SF2");
        var sf = new GuitPySynth.Sf2.SoundFont();
        sf.Load(input);
        GuitPySynth.Util.Logger.Debug("Building patchbank");
        ...
        for (var $i4 = 0,$t4 = sf.Presets.SampleHeaders,$l4 = $t4.length,sampleHeader = $t4[$i4]; $i4 < $l4; $i4++, sampleHeader = $t4[$i4]){
            this._assets.SampleAssets.push(new GuitPySynth.Bank.SampleDataAsset(sampleHeader, sf.SampleData));
        }
        ...
        for (var $i5 = 0,$t5 = sf.Presets.PresetHeaders,$l5 = $t5.length,p = $t5[$i5]; $i5 < $l5; $i5++, p = $t5[$i5]){
            var globalGens = null;
            var i;
            if (p.Zones[0].Generators.length == 0 || p.Zones[0].Generators[p.Zones[0].Generators.length - 1].GeneratorType != GuitPySynth.Sf2.GeneratorEnum.Instrument){
            ...
            while (i < p.Zones.length){
                ...
                var presetLoVel = 0;
                var presetHiVel = 127;
                if (p.Zones[i].Generators[0].GeneratorType == GuitPySynth.Sf2.GeneratorEnum.KeyRange){
                    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
                    ...
                    if (p.Zones[i].Generators.length > 1 && p.Zones[i].Generators[1].GeneratorType == GuitPySynth.Sf2.GeneratorEnum.VelocityRange){
                        if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
                ...
                else if (p.Zones[i].Generators[0].GeneratorType == GuitPySynth.Sf2.GeneratorEnum.VelocityRange){
                    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
                ...
                if (p.Zones[i].Generators[p.Zones[i].Generators.length - 1].GeneratorType == GuitPySynth.Sf2.GeneratorEnum.Instrument){
                    ...
                    for (var $i6 = 0,$l6 = insts.length,inst = insts[$i6]; $i6 < $l6; $i6++, inst = insts[$i6]){
                        ...
                        if ((instLoKey <= presetHiKey && presetLoKey <= instHiKey) && (instLoVel <= presetHiVel && presetLoVel <= instHiVel)){
                            var r = new GuitPySynth.Sf2.Sf2Region();
                            GuitPySynth.Platform.Std.ArrayCopy(inst.Generators, 0, r.Generators, 0, r.Generators.length);
            ...
            var mp = new GuitPySynth.Bank.Patch.MultiPatch(p.Name);
            mp.LoadSf2(regionList.slice(0), this._assets);
            this._assets.PatchAssets.push(new GuitPySynth.Bank.PatchAsset(mp.Name, mp));
    ...
    ReadSf2Instruments: function (instruments){
        ...
        for (var x = 0; x < instruments.length; x++){
            var globalGens = null;
            var i;
            if (instruments[x].Zones[0].Generators.length == 0 || instruments[x].Zones[0].Generators[instruments[x].Zones[0].Generators.length - 1].GeneratorType != GuitPySynth.Sf2.GeneratorEnum.SampleID){
        ...
        if (!isRelative){
            ...
            for (var x = 0; x < genList.length; x++){
                ...
                if (value < 5 || value == 12 || value == 45 || value == 46 || value == 47 || value == 50 || value == 54 || value == 57 || value == 58){
                    ...
                    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
                        region.Generators[value] = GuitPySynth.Platform.TypeUtils.ToInt16((lo_a | (hi_a << 8)));
                    }
                    else {
                        region.Generators[value] = GuitPySynth.Platform.TypeUtils.ToInt16((lo_a << 8) | hi_a);
                    }
                }
                else {
                    region.Generators[value] = GuitPySynth.Platform.TypeUtils.ToInt16(region.Generators[value] + genList[x].get_AmountInt16());
                }
            }
        }
    },
...
$StaticConstructor(function (){
    GuitPySynth.Bank.PatchBank.DrumBank = 128;
    GuitPySynth.Bank.PatchBank.BankSize = 128;
});
GuitPySynth.Bank.PatchAsset = function (name, patch){
    this.Name = null;
    this.Patch = null;
    this.Name = name;
    this.Patch = patch;
};
GuitPySynth.Ds = GuitPySynth.Ds || {};
GuitPySynth.Ds.LinkedList = function (){
    this.First = null;
    this.Length = 0;
    this.Length = 0;
};
GuitPySynth.Ds.LinkedList.prototype = {
    AddFirst: function (value){
        var node = new GuitPySynth.Ds.LinkedListNode();
        node.Value = value;
        if (this.First == null){
...
GuitPySynth.Ds.LinkedListNode = function (){
    this._list = null;
    this._next = null;
    this._prev = null;
    this.Value = null;
};
GuitPySynth.Ds.LinkedListNode.prototype = {
    get_Next: function (){
...
GuitPySynth.Ds.CircularSampleBuffer = function (size){
    this._buffer = null;
...
GuitPySynth.Ds.CircularSampleBuffer.prototype = {
    get_Count: function (){
...
GuitPySynth.IO = GuitPySynth.IO || {};
GuitPySynth.IO.ByteBuffer = function (){
    this._buffer = null;
    this._capacity = 0;
    this.Length = 0;
    this.Position = 0;
};
GuitPySynth.IO.ByteBuffer.prototype = {
    GetBuffer: function (){
        return this._buffer;
    },
    Reset: function (){
...
GuitPySynth.IO.ByteBuffer.Empty = function (){
    return GuitPySynth.IO.ByteBuffer.WithCapactiy(0);
};
GuitPySynth.IO.ByteBuffer.WithCapactiy = function (capacity){
    var buffer = new GuitPySynth.IO.ByteBuffer();
    buffer._buffer = new Uint8Array(capacity);
    buffer._capacity = capacity;
    return buffer;
};
GuitPySynth.IO.ByteBuffer.FromBuffer = function (data){
    var buffer = new GuitPySynth.IO.ByteBuffer();
    buffer._buffer = data;
    buffer._capacity = buffer.Length = data.length;
    return buffer;
};
GuitPySynth.MidiFileSequencer = function (synthesizer){
    this._synthesizer = null;
...
GuitPySynth.MidiFileSequencer.prototype = {
    get_PlaybackRange: function (){
        return this._playbackRange;
    },
    set_PlaybackRange: function (value){
    ...
    LoadMidi: function (midiFile){
        ...
        for (var x = 0; x < midiFile.Tracks[0].MidiEvents.length; x++){
            ...
            var synthData = new GuitPySynth.Synthesis.SynthEvent(mEvent);
            ...
            if (mEvent.get_Command() == GuitPySynth.Midi.Event.MidiEventTypeEnum.Meta && mEvent.get_Data1() == 81){
                var meta = mEvent;
                bpm = 60000000 / meta.Value;
                this._tempoChanges.push(new GuitPySynth.MidiFileSequencerTempoChange(bpm, absTick, ((absTime)) | 0));
            }
            else if (mEvent.get_Command() == GuitPySynth.Midi.Event.MidiEventTypeEnum.Meta && mEvent.get_Data1() == 88){
                var meta = mEvent;
                var timeSignatureDenominator = (Math.pow(2, meta.Data[1])) | 0;
                metronomeLength = ((this._division * (4 / timeSignatureDenominator))) | 0;
            }
            else if (mEvent.get_Command() == GuitPySynth.Midi.Event.MidiEventTypeEnum.ProgramChange){
            ...
            if (metronomeLength > 0){
                while (metronomeTick < absTick){
                    var metronome = GuitPySynth.Synthesis.SynthEvent.NewMetronomeEvent(metronomeLength);
...
GuitPySynth.MidiFileSequencerTempoChange = function (bpm, ticks, time){
    this.Bpm = 0;
    this.Ticks = 0;
    this.Time = 0;
    this.Bpm = bpm;
...
GuitPySynth.Midi = GuitPySynth.Midi || {};
GuitPySynth.Midi.Event = GuitPySynth.Midi.Event || {};
GuitPySynth.Midi.Event.MidiEvent = function (delta, status, data1, data2){
    this.Message = 0;
    this.DeltaTime = 0;
    this.DeltaTime = delta;
    this.Message = status | (data1 << 8) | (data2 << 16);
};
GuitPySynth.Midi.Event.MidiEvent.prototype = {
...
GuitPySynth.Midi.Event.SystemCommonEvent = function (delta, status, data1, data2){
    GuitPySynth.Midi.Event.MidiEvent.call(this, delta, status, data1, data2);
};
GuitPySynth.Midi.Event.SystemCommonEvent.prototype = {
    get_Channel: function (){
        return -1;
    },
    get_Command: function (){
...
$Inherit(GuitPySynth.Midi.Event.SystemCommonEvent, GuitPySynth.Midi.Event.MidiEvent);
GuitPySynth.Midi.Event.SystemExclusiveEvent = function (delta, status, id, data){
    this.Data = null;
    GuitPySynth.Midi.Event.SystemCommonEvent.call(this, delta, status, (id & 255), (id >> 8));
    this.Data = data;
};
GuitPySynth.Midi.Event.SystemExclusiveEvent.prototype = {
    get_ManufacturerId: function (){
        return this.Message >> 8;
    }
};
$Inherit(GuitPySynth.Midi.Event.SystemExclusiveEvent, GuitPySynth.Midi.Event.SystemCommonEvent);
GuitPySynth.Midi.Event.SystemCommonTypeEnum = {
    SystemExclusive: 240,
    MtcQuarterFrame: 241,
    SongPosition: 242,
    SongSelect: 243,
...
GuitPySynth.Midi.Event.RealTimeTypeEnum = {
    MidiClock: 248,
    MidiTick: 249,
    MidiStart: 250,
    MidiContinue: 252,
...
GuitPySynth.Midi.Event.RealTimeEvent = function (delta, status, data1, data2){
    GuitPySynth.Midi.Event.MidiEvent.call(this, delta, status, data1, data2);
};
GuitPySynth.Midi.Event.RealTimeEvent.prototype = {
    get_Channel: function (){
        return -1;
    },
    get_Command: function (){
...
$Inherit(GuitPySynth.Midi.Event.RealTimeEvent, GuitPySynth.Midi.Event.MidiEvent);
GuitPySynth.Midi.Event.MidiEventTypeEnum = {
    NoteOff: 128,
    NoteOn: 144,
    NoteAftertouch: 160,
    Controller: 176,
...
GuitPySynth.Midi.Event.ControllerTypeEnum = {
    BankSelectCoarse: 0,
    ModulationCoarse: 1,
...
GuitPySynth.Midi.Event.MetaEvent = function (delta, status, data1, data2){
    GuitPySynth.Midi.Event.MidiEvent.call(this, delta, status, data1, data2);
};
GuitPySynth.Midi.Event.MetaEvent.prototype = {
    get_Channel: function (){
        return -1;
    },
    get_Command: function (){
...
$Inherit(GuitPySynth.Midi.Event.MetaEvent, GuitPySynth.Midi.Event.MidiEvent);
GuitPySynth.Midi.Event.MetaTextEvent = function (delta, status, metaId, text){
    this.Text = null;
    GuitPySynth.Midi.Event.MetaEvent.call(this, delta, status, metaId, 0);
    this.Text = text;
};
$Inherit(GuitPySynth.Midi.Event.MetaTextEvent, GuitPySynth.Midi.Event.MetaEvent);
GuitPySynth.Midi.Event.MetaNumberEvent = function (delta, status, metaId, number){
    this.Value = 0;
    GuitPySynth.Midi.Event.MetaEvent.call(this, delta, status, metaId, 0);
    this.Value = number;
};
$Inherit(GuitPySynth.Midi.Event.MetaNumberEvent, GuitPySynth.Midi.Event.MetaEvent);
GuitPySynth.Midi.Event.MetaEventTypeEnum = {
    SequenceNumber: 0,
    TextEvent: 1,
    CopyrightNotice: 2,
...
GuitPySynth.Midi.Event.MetaDataEvent = function (delta, status, metaId, data){
    this.Data = null;
    GuitPySynth.Midi.Event.MetaEvent.call(this, delta, status, metaId, 0);
    this.Data = data;
};
$Inherit(GuitPySynth.Midi.Event.MetaDataEvent, GuitPySynth.Midi.Event.MetaEvent);
GuitPySynth.Midi.MidiTrack = function (instPrograms, drumPrograms, activeChannels, midiEvents){
...
GuitPySynth.Midi.MidiHelper = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Midi.MidiHelper.MicroSecondsPerMinute = 60000000;
    GuitPySynth.Midi.MidiHelper.MinChannel = 0;
    GuitPySynth.Midi.MidiHelper.MaxChannel = 15;
    GuitPySynth.Midi.MidiHelper.DrumChannel = 9;
});
GuitPySynth.Midi.MidiFile = function (){
    this.Division = 0;
    this.TrackFormat = GuitPySynth.Midi.MidiTrackFormat.SingleTrack;
    this.TimingStandard = GuitPySynth.Midi.MidiTimeFormat.TicksPerBeat;
    this.Tracks = null;
    this.Division = 0;
    this.TrackFormat = GuitPySynth.Midi.MidiTrackFormat.SingleTrack;
    this.TimingStandard = GuitPySynth.Midi.MidiTimeFormat.TicksPerBeat;
};
GuitPySynth.Midi.MidiFile.prototype = {
    CombineTracks: function (){
        ...
        GuitPySynth.Platform.TypeUtils.ClearIntArray(counters);
    ...
    MergeTracks: function (){
        ...
        var track = new GuitPySynth.Midi.MidiTrack(programsUsed.slice(0), drumProgramsUsed.slice(0), channelsUsed.slice(0), new Array(eventCount));
    ...
    ReadHeader: function (input){
        if (GuitPySynth.Util.IOHelper.ReadInt32BE(input) != 6)
            throw $CreateException(new System.Exception.ctor$$String("Midi header is invalid."), new Error());
        this.TrackFormat = GuitPySynth.Util.IOHelper.ReadInt16BE(input);
        this.Tracks = new Array(GuitPySynth.Util.IOHelper.ReadInt16BE(input));
        var div = GuitPySynth.Util.IOHelper.ReadInt16BE(input);
        this.Division = div & 32767;
        this.TimingStandard = ((div & 32768) > 0) ? GuitPySynth.Midi.MidiTimeFormat.FramesPerSecond : GuitPySynth.Midi.MidiTimeFormat.TicksPerBeat;
    },
    ReadTrack: function (input){
        ...
        var noteOnCount = 0;
        var totalTime = 0;
        while (GuitPySynth.Util.IOHelper.Read8BitChars(input, 4) != "MTrk"){
            var length = GuitPySynth.Util.IOHelper.ReadInt32BE(input);
            while (length > 0){
                length--;
                input.ReadByte();
            }
        }
        var endPosition = GuitPySynth.Util.IOHelper.ReadInt32BE(input) + input.Position;
        var prevStatus = 0;
        while (input.Position < endPosition){
            var delta = GuitPySynth.Midi.MidiFile.ReadVariableLength(input);
            totalTime += delta;
            var status = input.ReadByte();
            if (status >= 128 && status <= 239){
                //voice message
                prevStatus = status;
                eventList.push(GuitPySynth.Midi.MidiFile.ReadVoiceMessage(input, delta, status, input.ReadByte()));
                noteOnCount = GuitPySynth.Midi.MidiFile.TrackVoiceStats(eventList[eventList.length - 1], instList, drumList, channelList, noteOnCount);
            }
            else if (status >= 240 && status <= 247){
                //system common message
                prevStatus = 0;
                eventList.push(GuitPySynth.Midi.MidiFile.ReadSystemCommonMessage(input, delta, status));
            }
            else if (status >= 248 && status <= 255){
                //realtime message
                eventList.push(GuitPySynth.Midi.MidiFile.ReadRealTimeMessage(input, delta, status));
            }
            else {
                //data bytes
                if (prevStatus == 0){
                    ...
                    eventList.push(GuitPySynth.Midi.MidiFile.ReadVoiceMessage(input, delta, prevStatus, status));
                    noteOnCount = GuitPySynth.Midi.MidiFile.TrackVoiceStats(eventList[eventList.length - 1], instList, drumList, channelList, noteOnCount);
                }
            }
        }
        ...
        var track = new GuitPySynth.Midi.MidiTrack(instList.slice(0), drumList.slice(0), channelList.slice(0), eventList.slice(0));
        track.NoteOnCount = noteOnCount;
        track.EndTime = totalTime;
        return track;
    }
};
GuitPySynth.Midi.MidiFile.ReadMetaMessage = function (input, delta, status){
    var metaStatus = input.ReadByte();
    switch (metaStatus){
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.SequenceNumber:
            {
            var count = input.ReadByte();
            if (count == 0)
                return new GuitPySynth.Midi.Event.MetaNumberEvent(delta, status, metaStatus, -1);
            else if (count == 2){
                return new GuitPySynth.Midi.Event.MetaNumberEvent(delta, status, metaStatus, GuitPySynth.Util.IOHelper.ReadInt16BE(input));
            }
            else
                throw $CreateException(new System.Exception.ctor$$String("Invalid sequence number event."), new Error());
            }
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.TextEvent:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.CopyrightNotice:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.SequenceOrTrackName:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.InstrumentName:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.LyricText:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.MarkerText:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.CuePoint:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.PatchName:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.PortName:
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, GuitPySynth.Midi.MidiFile.ReadString(input));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.MidiChannel:
            if (input.ReadByte() != 1)
            throw $CreateException(new System.Exception.ctor$$String("Invalid midi channel event. Expected size of 1."), new Error());
            return new GuitPySynth.Midi.Event.MetaEvent(delta, status, metaStatus, input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.MidiPort:
            if (input.ReadByte() != 1)
            throw $CreateException(new System.Exception.ctor$$String("Invalid midi port event. Expected size of 1."), new Error());
            return new GuitPySynth.Midi.Event.MetaEvent(delta, status, metaStatus, input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.EndOfTrack:
            return new GuitPySynth.Midi.Event.MetaEvent(delta, status, metaStatus, input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.Tempo:
            if (input.ReadByte() != 3)
            throw $CreateException(new System.Exception.ctor$$String("Invalid tempo event. Expected size of 3."), new Error());
            return new GuitPySynth.Midi.Event.MetaNumberEvent(delta, status, metaStatus, (input.ReadByte() << 16) | (input.ReadByte() << 8) | input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.SmpteOffset:
            if (input.ReadByte() != 5)
            throw $CreateException(new System.Exception.ctor$$String("Invalid smpte event. Expected size of 5."), new Error());
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, input.ReadByte() + ":" + input.ReadByte() + ":" + input.ReadByte() + ":" + input.ReadByte() + ":" + input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.TimeSignature:
            if (input.ReadByte() != 4)
            throw $CreateException(new System.Exception.ctor$$String("Invalid time signature event. Expected size of 4."), new Error());
            return new GuitPySynth.Midi.Event.MetaDataEvent(delta, status, metaStatus, new Uint8Array([input.ReadByte(), input.ReadByte(), input.ReadByte(), input.ReadByte()]));
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.KeySignature:
            if (input.ReadByte() != 2)
            throw $CreateException(new System.Exception.ctor$$String("Invalid key signature event. Expected size of 2."), new Error());
            return new GuitPySynth.Midi.Event.MetaTextEvent(delta, status, metaStatus, input.ReadByte() + ":" + input.ReadByte());
        case GuitPySynth.Midi.Event.MetaEventTypeEnum.SequencerSpecific:
            var length = GuitPySynth.Midi.MidiFile.ReadVariableLength(input);
            var data = GuitPySynth.Util.IOHelper.ReadByteArray(input, length);
            return new GuitPySynth.Midi.Event.MetaDataEvent(delta, status, metaStatus, data);
    }
    throw $CreateException(new System.Exception.ctor$$String("Not a valid meta message Status: " + status + " Meta: " + metaStatus), new Error());
};
GuitPySynth.Midi.MidiFile.ReadRealTimeMessage = function (input, delta, status){
    switch (status){
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.MidiClock:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.MidiTick:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.MidiStart:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.MidiContinue:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.MidiStop:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.ActiveSense:
            return new GuitPySynth.Midi.Event.RealTimeEvent(delta, status, 0, 0);
        case GuitPySynth.Midi.Event.RealTimeTypeEnum.Reset:
            return GuitPySynth.Midi.MidiFile.ReadMetaMessage(input, delta, status);
        default:
            throw $CreateException(new System.Exception.ctor$$String("The real time message was invalid or unsupported : " + status), new Error());
    }
};
GuitPySynth.Midi.MidiFile.ReadSystemCommonMessage = function (input, delta, status){
    switch (status){
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.SystemExclusive2:
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.SystemExclusive:
            {
            var maker = GuitPySynth.Util.IOHelper.ReadInt16BE(input);
            if (maker == 0){
                maker = GuitPySynth.Util.IOHelper.ReadInt16BE(input);
            }
            else if (maker == 247)
                return null;
            var data = [];
            ...
            return new GuitPySynth.Midi.Event.SystemExclusiveEvent(delta, status, maker, data.slice(0));
            }
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.MtcQuarterFrame:
            return new GuitPySynth.Midi.Event.SystemCommonEvent(delta, status, input.ReadByte(), 0);
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.SongPosition:
            return new GuitPySynth.Midi.Event.SystemCommonEvent(delta, status, input.ReadByte(), input.ReadByte());
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.SongSelect:
            return new GuitPySynth.Midi.Event.SystemCommonEvent(delta, status, input.ReadByte(), 0);
        case GuitPySynth.Midi.Event.SystemCommonTypeEnum.TuneRequest:
            return new GuitPySynth.Midi.Event.SystemCommonEvent(delta, status, 0, 0);
        default:
...
GuitPySynth.Midi.MidiFile.ReadVoiceMessage = function (input, delta, status, data1){
    switch ((status & 240)){
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.NoteOff:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, input.ReadByte());
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.NoteOn:
            var velocity = input.ReadByte();
            if (velocity == 0)
            status = ((status & 15) | 128);
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, velocity);
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.NoteAftertouch:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, input.ReadByte());
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.Controller:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, input.ReadByte());
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.ProgramChange:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, 0);
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.ChannelAftertouch:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, 0);
        case GuitPySynth.Midi.Event.MidiEventTypeEnum.PitchBend:
            return new GuitPySynth.Midi.Event.MidiEvent(delta, status, data1, input.ReadByte());
        default:
            throw $CreateException(new System.Exception.ctor$$String("The status provided was not that of a voice message."), new Error());
    }
};
GuitPySynth.Midi.MidiFile.TrackVoiceStats = function (midiEvent, instList, drumList, channelList, noteOnCount){
    if (midiEvent.get_Command() == GuitPySynth.Midi.Event.MidiEventTypeEnum.NoteOn){
        var chan = midiEvent.get_Channel();
        if (Array.prototype.indexOf.call(channelList,chan) == -1)
            channelList.push(chan);
        noteOnCount++;
    }
    else if (midiEvent.get_Command() == GuitPySynth.Midi.Event.MidiEventTypeEnum.ProgramChange){
        var chan = midiEvent.get_Channel();
...
GuitPySynth.Midi.MidiFile.ReadVariableLength = function (input){
    var value = 0;
    var next;
    do{
        next = input.ReadByte();
...
GuitPySynth.Midi.MidiFile.ReadString = function (input){
    var length = GuitPySynth.Midi.MidiFile.ReadVariableLength(input);
    return GuitPySynth.Util.IOHelper.Read8BitChars(input, length);
    // TODO: check for correct string encoding
};
GuitPySynth.Midi.MidiTrackFormat = {
    SingleTrack: 0,
    MultiTrack: 1,
    MultiSong: 2
};
GuitPySynth.Midi.MidiTimeFormat = {
    TicksPerBeat: 0,
    FramesPerSecond: 1
};
GuitPySynth.PlayerState = {
    Paused: 0,
    Playing: 1
};
GuitPySynth.Sf2 = GuitPySynth.Sf2 || {};
GuitPySynth.Sf2.Chunks = GuitPySynth.Sf2.Chunks || {};
GuitPySynth.Sf2.Chunks.Chunk = function (id, size){
    this.Id = null;
    this.Size = 0;
    this.Id = id;
    this.Size = size;
};
GuitPySynth.Sf2.Chunks.ZoneChunk = function (id, size, input){
    this._zoneData = null;
    GuitPySynth.Sf2.Chunks.Chunk.call(this, id, size);
    this._zoneData = new Array((((size / 4)) | 0));
    var lastZone = null;
    for (var x = 0; x < this._zoneData.length; x++){
        var z = new GuitPySynth.Sf2.Chunks.RawZoneData();
        z.GeneratorIndex = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
        z.ModulatorIndex = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
        if (lastZone != null){
...
GuitPySynth.Sf2.Chunks.ZoneChunk.prototype = {
    ToZones: function (modulators, generators){
        ...
        for (var x = 0; x < zones.length; x++){
            ...
            var zone = new GuitPySynth.Sf2.Zone();
            zone.Generators = new Array(rawZone.GeneratorCount);
            GuitPySynth.Platform.Std.ArrayCopy(generators, rawZone.GeneratorIndex, zone.Generators, 0, rawZone.GeneratorCount);
            zone.Modulators = new Array(rawZone.ModulatorCount);
            GuitPySynth.Platform.Std.ArrayCopy(modulators, rawZone.ModulatorIndex, zone.Modulators, 0, rawZone.ModulatorCount);
            zones[x] = zone;
        }
        return zones;
    }
};
$Inherit(GuitPySynth.Sf2.Chunks.ZoneChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Chunks.RawZoneData = function (){
    this.GeneratorIndex = 0;
    this.ModulatorIndex = 0;
    this.GeneratorCount = 0;
    this.ModulatorCount = 0;
};
GuitPySynth.Sf2.Chunks.SampleHeaderChunk = function (id, size, input){
    ...
    for (var x = 0; x < this.SampleHeaders.length; x++){
        this.SampleHeaders[x] = new GuitPySynth.Sf2.SampleHeader(input);
    }
    new GuitPySynth.Sf2.SampleHeader(input);
    //read terminal record
};
$Inherit(GuitPySynth.Sf2.Chunks.SampleHeaderChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Chunks.PresetHeaderChunk = function (id, size, input){
    ...
    for (var x = 0; x < this._rawPresets.length; x++){
        var p = new GuitPySynth.Sf2.Chunks.RawPreset();
        p.Name = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, 20);
        p.PatchNumber = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
        p.BankNumber = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
        p.StartPresetZoneIndex = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
        p.Library = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
        p.Genre = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
        p.Morphology = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
        if (lastPreset != null){
...
GuitPySynth.Sf2.Chunks.PresetHeaderChunk.prototype = {
    ToPresets: function (presetZones){
        ...
        for (var x = 0; x < presets.length; x++){
            ...
            var p = new GuitPySynth.Sf2.PresetHeader();
            ...
            GuitPySynth.Platform.Std.ArrayCopy(presetZones, rawPreset.StartPresetZoneIndex, p.Zones, 0, p.Zones.length);
            presets[x] = p;
        }
        return presets;
    }
};
$Inherit(GuitPySynth.Sf2.Chunks.PresetHeaderChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Chunks.RawPreset = function (){
    this.Name = null;
    this.PatchNumber = 0;
    this.BankNumber = 0;
...
GuitPySynth.Sf2.Chunks.ModulatorChunk = function (id, size, input){
    ...
    for (var x = 0; x < this.Modulators.length; x++){
        this.Modulators[x] = new GuitPySynth.Sf2.Modulator(input);
    }
    new GuitPySynth.Sf2.Modulator(input);
};
$Inherit(GuitPySynth.Sf2.Chunks.ModulatorChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Chunks.InstrumentChunk = function (id, size, input){
    ...
    for (var x = 0; x < this._rawInstruments.length; x++){
        var i = new GuitPySynth.Sf2.Chunks.RawInstrument();
        i.Name = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, 20);
        i.StartInstrumentZoneIndex = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
...
GuitPySynth.Sf2.Chunks.InstrumentChunk.prototype = {
    ToInstruments: function (zones){
        ...
        for (var x = 0; x < inst.length; x++){
            ...
            var i = new GuitPySynth.Sf2.Instrument();
            i.Name = rawInst.Name;
            i.Zones = new Array(rawInst.EndInstrumentZoneIndex - rawInst.StartInstrumentZoneIndex + 1);
            GuitPySynth.Platform.Std.ArrayCopy(zones, rawInst.StartInstrumentZoneIndex, i.Zones, 0, i.Zones.length);
            inst[x] = i;
        }
        return inst;
    }
};
$Inherit(GuitPySynth.Sf2.Chunks.InstrumentChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Chunks.RawInstrument = function (){
    this.Name = null;
    this.StartInstrumentZoneIndex = 0;
    this.EndInstrumentZoneIndex = 0;
};
GuitPySynth.Sf2.Chunks.GeneratorChunk = function (id, size, input){
    ...
    for (var x = 0; x < this.Generators.length; x++){
        this.Generators[x] = new GuitPySynth.Sf2.Generator(input);
    }
    new GuitPySynth.Sf2.Generator(input);
    // terminal record
};
$Inherit(GuitPySynth.Sf2.Chunks.GeneratorChunk, GuitPySynth.Sf2.Chunks.Chunk);
GuitPySynth.Sf2.Modulator = function (input){
    this._sourceModulationData = null;
    this._destinationGenerator = 0;
    this._amount = 0;
    this._sourceModulationAmount = null;
    this._sourceTransform = 0;
    this._sourceModulationData = new GuitPySynth.Sf2.ModulatorType(input);
    this._destinationGenerator = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
    this._amount = GuitPySynth.Util.IOHelper.ReadInt16LE(input);
    this._sourceModulationAmount = new GuitPySynth.Sf2.ModulatorType(input);
    this._sourceTransform = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
};
GuitPySynth.Sf2.Zone = function (){
    this.Modulators = null;
    this.Generators = null;
};
GuitPySynth.Sf2.SoundFontSampleData = function (input){
    this.BitsPerSample = 0;
    this.SampleData = null;
    var id = GuitPySynth.Util.IOHelper.Read8BitChars(input, 4);
    var size = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    ...
    id = GuitPySynth.Util.IOHelper.Read8BitChars(input, 4);
    ...
    while (input.Position < readTo){
        var subID = GuitPySynth.Util.IOHelper.Read8BitChars(input, 4);
        size = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
        switch (subID.toLowerCase()){
            case "smpl":
                this.BitsPerSample = 16;
                rawSampleData = GuitPySynth.Util.IOHelper.ReadByteArray(input, size);
                break;
            case "sm24":
...
GuitPySynth.Sf2.SoundFontPresets = function (input){
    this.SampleHeaders = null;
    ...
    while (input.Position < readTo){
        ...
        switch (id.toLowerCase()){
            case "phdr":
                phdr = new GuitPySynth.Sf2.Chunks.PresetHeaderChunk(id, size, input);
                break;
            case "pbag":
                pbag = new GuitPySynth.Sf2.Chunks.ZoneChunk(id, size, input);
                break;
            case "pmod":
                presetModulators = new GuitPySynth.Sf2.Chunks.ModulatorChunk(id, size, input).Modulators;
                break;
            case "pgen":
                presetGenerators = new GuitPySynth.Sf2.Chunks.GeneratorChunk(id, size, input).Generators;
                break;
            case "inst":
                inst = new GuitPySynth.Sf2.Chunks.InstrumentChunk(id, size, input);
                break;
            case "ibag":
                ibag = new GuitPySynth.Sf2.Chunks.ZoneChunk(id, size, input);
                break;
            case "imod":
                instrumentModulators = new GuitPySynth.Sf2.Chunks.ModulatorChunk(id, size, input).Modulators;
                break;
            case "igen":
                instrumentGenerators = new GuitPySynth.Sf2.Chunks.GeneratorChunk(id, size, input).Generators;
                break;
            case "shdr":
                this.SampleHeaders = new GuitPySynth.Sf2.Chunks.SampleHeaderChunk(id, size, input).SampleHeaders;
                break;
            default:
...
GuitPySynth.Sf2.SoundFontInfo = function (input){
    ...
    while (input.Position < readTo){
        ...
        switch (id.toLowerCase()){
            case "ifil":
                this.SfVersionMajor = GuitPySynth.Util.IOHelper.ReadInt16LE(input);
                this.SfVersionMinor = GuitPySynth.Util.IOHelper.ReadInt16LE(input);
                break;
            case "isng":
                this.SoundEngine = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "inam":
                this.BankName = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "irom":
                this.DataRom = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "iver":
                this.RomVersionMajor = GuitPySynth.Util.IOHelper.ReadInt16LE(input);
                this.RomVersionMinor = GuitPySynth.Util.IOHelper.ReadInt16LE(input);
                break;
            case "icrd":
                this.CreationDate = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "ieng":
                this.Author = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "iprd":
                this.TargetProduct = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "icop":
                this.Copyright = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "icmt":
                this.Comments = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            case "isft":
                this.Tools = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, size);
                break;
            default:
...
GuitPySynth.Sf2.SoundFont = function (){
    this.Info = null;
    this.SampleData = null;
    this.Presets = null;
};
GuitPySynth.Sf2.SoundFont.prototype = {
    Load: function (input){
        var id = GuitPySynth.Util.IOHelper.Read8BitChars(input, 4);
        var size = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
        ...
        GuitPySynth.Util.Logger.Debug("Reading info chunk");
        this.Info = new GuitPySynth.Sf2.SoundFontInfo(input);
        GuitPySynth.Util.Logger.Debug("Reading sampledata chunk");
        this.SampleData = new GuitPySynth.Sf2.SoundFontSampleData(input);
        GuitPySynth.Util.Logger.Debug("Reading preset chunk");
        this.Presets = new GuitPySynth.Sf2.SoundFontPresets(input);
    }
};
GuitPySynth.Sf2.Sf2Region = function (){
    this.Generators = null;
    this.Generators = new Int16Array(61);
};
GuitPySynth.Sf2.Sf2Region.prototype = {
...
GuitPySynth.Sf2.SampleHeader = function (input){
    this.Name = null;
    this.Start = 0;
    this.End = 0;
    this.StartLoop = 0;
    ...
    this.Tune = 0;
    this.SampleLink = 0;
    this.SoundFontSampleLink = GuitPySynth.Sf2.SFSampleLink.MonoSample;
    this.Name = GuitPySynth.Util.IOHelper.Read8BitStringLength(input, 20);
    this.Start = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    this.End = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    this.StartLoop = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    this.EndLoop = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    this.SampleRate = GuitPySynth.Util.IOHelper.ReadInt32LE(input);
    this.RootKey = input.ReadByte();
    this.Tune = GuitPySynth.Platform.TypeUtils.ToInt16(input.ReadByte());
    this.SampleLink = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
    this.SoundFontSampleLink = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
};
GuitPySynth.Sf2.PresetHeader = function (){
    this.Name = null;
    this.PatchNumber = 0;
    this.BankNumber = 0;
    this.Library = 0;
...
GuitPySynth.Sf2.ModulatorType = function (input){
    this._controllerSource = 0;
    this.Polarity = GuitPySynth.Sf2.PolarityEnum.Unipolar;
    this.Direction = GuitPySynth.Sf2.DirectionEnum.MinToMax;
    this.SourceType = 0;
    this.IsMidiContinuousController = false;
    var raw = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
    this.Polarity = (raw & 512) == 512 ? GuitPySynth.Sf2.PolarityEnum.Bipolar : GuitPySynth.Sf2.PolarityEnum.Unipolar;
    this.Direction = (raw & 256) == 256 ? GuitPySynth.Sf2.DirectionEnum.MaxToMin : GuitPySynth.Sf2.DirectionEnum.MinToMax;
    this.IsMidiContinuousController = ((raw & 128) == 128);
    this.SourceType = ((raw & (64512)) >> 10);
    this._controllerSource = ((raw & 127)&65535);
};
GuitPySynth.Sf2.Instrument = function (){
    this.Name = null;
    this.Zones = null;
};
GuitPySynth.Sf2.Generator = function (input){
    this._rawAmount = 0;
    this.GeneratorType = GuitPySynth.Sf2.GeneratorEnum.StartAddressOffset;
    this.GeneratorType = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
    this._rawAmount = GuitPySynth.Util.IOHelper.ReadUInt16LE(input);
};
GuitPySynth.Sf2.Generator.prototype = {
    get_AmountInt16: function (){
        return GuitPySynth.Platform.TypeUtils.ToInt16(this._rawAmount);
    },
...
GuitPySynth.Sf2.TransformEnum = {
    Linear: 0,
    AbsoluteValue: 2
};
GuitPySynth.Sf2.ControllerSourceEnum = {
    NoController: 0,
    NoteOnVelocity: 2,
    NoteOnKeyNumber: 3,
    PolyPressure: 10,
...
GuitPySynth.Sf2.DirectionEnum = {
    MinToMax: 0,
    MaxToMin: 1
};
GuitPySynth.Sf2.PolarityEnum = {
    Unipolar: 0,
    Bipolar: 1
};
GuitPySynth.Sf2.SourceTypeEnum = {
    Linear: 0,
    Concave: 1,
    Convex: 2,
    Switch: 3
};
GuitPySynth.Synthesis = GuitPySynth.Synthesis || {};
GuitPySynth.Synthesis.CCValue = function (coarse, fine){
    this._coarseValue = 0;
    this._fineValue = 0;
    this._combined = 0;
...
GuitPySynth.Synthesis.CCValue = function (combined){
    this._coarseValue = 0;
    this._fineValue = 0;
    this._combined = 0;
    this._coarseValue = 0;
...
GuitPySynth.Synthesis.CCValue.prototype = {
    get_Coarse: function (){
...
GuitPySynth.Synthesis.PlaybackRange = function (){
    this.StartTick = 0;
    this.EndTick = 0;
};
GuitPySynth.Synthesis.SynthParameters = function (synth){
    this.Program = 0;
    this.BankSelect = 0;
    ...
    this.MixVolume = 0;
    this.Synth = synth;
    this.Pan = new GuitPySynth.Synthesis.CCValue(0);
    this.Volume = new GuitPySynth.Synthesis.CCValue(0);
    this.Expression = new GuitPySynth.Synthesis.CCValue(0);
    this.ModRange = new GuitPySynth.Synthesis.CCValue(0);
    this.PitchBend = new GuitPySynth.Synthesis.CCValue(0);
    this.MasterFineTune = new GuitPySynth.Synthesis.CCValue(0);
    this.Rpn = new GuitPySynth.Synthesis.CCValue(0);
    this.MixVolume = 1;
    this.CurrentPan = new GuitPySynth.Bank.Components.PanComponent();
    this.ResetControllers();
};
GuitPySynth.Synthesis.SynthParameters.prototype = {
...
GuitPySynth.Synthesis.VoiceParameters = function (){
    this.mix1 = 0;
    this.mix2 = 0;
    this.Channel = 0;
    this.Note = 0;
    ...
    this.State = GuitPySynth.Synthesis.VoiceStateEnum.Stopped;
    this.PitchOffset = 0;
    this.VolOffset = 0;
    ...
    for (var x = 0; x < 4; x++){
        this.GeneratorParams[x] = new GuitPySynth.Bank.Components.Generators.GeneratorParameters();
        this.Envelopes[x] = new GuitPySynth.Bank.Components.Envelope();
        this.Filters[x] = new GuitPySynth.Bank.Components.Filter();
        this.Lfos[x] = new GuitPySynth.Bank.Components.Lfo();
    }
};
GuitPySynth.Synthesis.VoiceParameters.prototype = {
...
GuitPySynth.Synthesis.VoiceNode = function (){
    this.Value = null;
    this.Next = null;
};
GuitPySynth.Synthesis.VoiceManager = function (voiceCount){
    ...
    this._vNodes = new GuitPySynth.Ds.LinkedList();
    this.FreeVoices = new GuitPySynth.Ds.LinkedList();
    this.ActiveVoices = new GuitPySynth.Ds.LinkedList();
    for (var i = 0; i < voiceCount; i++){
        var v = new GuitPySynth.Synthesis.Voice();
        this._voicePool[i] = v;
        this._vNodes.AddLast(new GuitPySynth.Synthesis.VoiceNode());
...
GuitPySynth.Synthesis.VoiceManager.prototype = {
    ...
    StealQuietestVoice: function (){
        ...
        var node = this.ActiveVoices.First;
        while (node != null){
            if (node.Value.VoiceParams.State != GuitPySynth.Synthesis.VoiceStateEnum.Playing){
        ...
        quietest.Value.VoiceParams.State = GuitPySynth.Synthesis.VoiceStateEnum.Stopped;
        return quietest.Value;
    }
};
GuitPySynth.Synthesis.VoiceStateEnum = {
    Stopped: 0,
    Stopping: 1,
    Playing: 2
};
GuitPySynth.Synthesis.Voice = function (){
    this.Patch = null;
    this.VoiceParams = null;
    this.VoiceParams = new GuitPySynth.Synthesis.VoiceParameters();
};
GuitPySynth.Synthesis.Voice.prototype = {
    Start: function (){
        if (this.VoiceParams.State != GuitPySynth.Synthesis.VoiceStateEnum.Stopped)
            return;
        if (this.Patch.Start(this.VoiceParams))
            this.VoiceParams.State = GuitPySynth.Synthesis.VoiceStateEnum.Playing;
    },
    Stop: function (){
        if (this.VoiceParams.State != GuitPySynth.Synthesis.VoiceStateEnum.Playing)
            return;
        this.VoiceParams.State = GuitPySynth.Synthesis.VoiceStateEnum.Stopping;
        this.Patch.Stop(this.VoiceParams);
    },
    StopImmediately: function (){
        this.VoiceParams.State = GuitPySynth.Synthesis.VoiceStateEnum.Stopped;
    },
...
GuitPySynth.Synthesis.SynthHelper = function (){
};
GuitPySynth.Synthesis.SynthHelper.ClampB = function (value, min, max){
    if (value <= min)
        return min;
    else if (value >= max)
        return max;
...
GuitPySynth.Synthesis.SynthHelper.ClampD = function (value, min, max){
    if (value <= min)
        return min;
    else if (value >= max)
        return max;
...
GuitPySynth.Synthesis.SynthHelper.ClampF = function (value, min, max){
    if (value <= min)
        return min;
    else if (value >= max)
        return max;
...
GuitPySynth.Synthesis.SynthHelper.ClampI = function (value, min, max){
    if (value <= min)
        return min;
    else if (value >= max)
        return max;
...
GuitPySynth.Synthesis.SynthHelper.ClampS = function (value, min, max){
    if (value <= min)
        return min;
    else if (value >= max)
        return max;
...
GuitPySynth.Synthesis.SynthHelper.NearestPowerOfTwo = function (value){
    return Math.pow(2, Math.Round(Math.Log(value, 2)));
};
GuitPySynth.Synthesis.SynthHelper.SamplesFromTime = function (sampleRate, seconds){
    return sampleRate * seconds;
};
GuitPySynth.Synthesis.SynthHelper.TimeFromSamples = function (sampleRate, samples){
    return samples / sampleRate;
};
GuitPySynth.Synthesis.SynthHelper.DBtoLinear = function (dBvalue){
    return Math.pow(10, (dBvalue / 20));
};
GuitPySynth.Synthesis.SynthHelper.LineartoDB = function (linearvalue){
    return 20 * (Math.log(linearvalue)/Math.LN10);
};
GuitPySynth.Synthesis.SynthHelper.FrequencyToKey = function (frequency, rootkey){
    return 12 * Math.Log(frequency / 440, 2) + rootkey;
};
GuitPySynth.Synthesis.SynthHelper.KeyToFrequency = function (key, rootkey){
    return Math.pow(2, (key - rootkey) / 12) * 440;
};
GuitPySynth.Synthesis.SynthHelper.SemitoneToPitch = function (key){
    //does not return a frequency, only the 2^(1/12) value.
    if (key < -127)
        key = -127;
    else if (key > 127)
        key = 127;
    return GuitPySynth.Util.Tables.SemitoneTable(127 + key);
};
GuitPySynth.Synthesis.SynthHelper.CentsToPitch = function (cents){
    //does not return a frequency, only the 2^(1/12) value.
    var key = (cents / 100) | 0;
    cents -= key * 100;
    if (key < -127)
        ...
        key = 127;
    return GuitPySynth.Util.Tables.SemitoneTable(127 + key) * GuitPySynth.Util.Tables.CentTable(100 + cents);
};
GuitPySynth.Synthesis.SynthEvent = function (e){
    this.Event = null;
    this.IsMetronome = false;
    this.Delta = 0;
    this.Event = e;
};
GuitPySynth.Synthesis.SynthEvent.NewMetronomeEvent = function (metronomeLength){
    var x = new GuitPySynth.Synthesis.SynthEvent(null);
    x.IsMetronome = true;
    return x;
};
GuitPySynth.Synthesis.Synthesizer = function (sampleRate, audioChannels, bufferSize, bufferCount, polyphony){
    ...
    this.MasterVolume = 1;
    this.SampleRate = GuitPySynth.Synthesis.SynthHelper.ClampI(sampleRate, MinSampleRate, MaxSampleRate);
    this.MicroBufferSize = GuitPySynth.Synthesis.SynthHelper.ClampI(bufferSize, ((0.001 * sampleRate)) | 0, ((0.05 * sampleRate)) | 0);
    ...
    for (var x = 0; x < this._synthChannels.length; x++){
        this._synthChannels[x] = new GuitPySynth.Synthesis.SynthParameters(this);
    }
    // setup metronome channel
    this._metronomeChannel = this._synthChannels.length - 1;
    // Create synth voices
    this._voiceManager = new GuitPySynth.Synthesis.VoiceManager(GuitPySynth.Synthesis.SynthHelper.ClampI(polyphony, 5, 250));
    // Create midi containers
    this._midiEventQueue = new GuitPySynth.Ds.LinkedList();
...
GuitPySynth.Synthesis.Synthesizer.prototype = {
    get_MetronomeVolume: function (){
    ...
    FillWorkingBuffer: function (){
        ...
        for (var x = 0; x < this.MicroBufferCount; x++){
            ...
            while (node != null){
                ...
                if (node.Value.VoiceParams.State == GuitPySynth.Synthesis.VoiceStateEnum.Stopped){
                    var delnode = node;
        ...
        GuitPySynth.Platform.TypeUtils.ClearIntArray(this._midiEventCounts);
    },
    ...
    NoteOffAll: function (immediate){
        ...
        if (immediate){
            ...
            while (node != null){
                ...
                if (voiceParams.State == GuitPySynth.Synthesis.VoiceStateEnum.Playing){
    ...
    ProcessMidiMessage: function (e){
        ...
        switch (command){
            ...
            case GuitPySynth.Midi.Event.MidiEventTypeEnum.Controller:
                switch (data1){
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.BankSelectCoarse:
                    if (channel == 9)
                    data2 += 128;
                    ...
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.LegatoPedal:
                    this._synthChannels[channel].LegatoPedal = data2 > 63;
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.NonRegisteredParameterCourse:
                    this._synthChannels[channel].Rpn.set_Combined(16383);
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.NonRegisteredParameterFine:
                    this._synthChannels[channel].Rpn.set_Combined(16383);
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.RegisteredParameterCourse:
                    this._synthChannels[channel].Rpn.set_Coarse(data2);
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.RegisteredParameterFine:
                    this._synthChannels[channel].Rpn.set_Fine(data2);
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.AllNotesOff:
                    this.NoteOffAll(false);
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.DataEntryCoarse:
                    ...
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.DataEntryFine:
                    switch (this._synthChannels[channel].Rpn.get_Combined()){
                        case 0:
                    ...
                    break;
                    case GuitPySynth.Midi.Event.ControllerTypeEnum.ResetControllers:
...
GuitPySynth.Util.PcmData = function (bits, pcmData, isDataInLittleEndianFormat){
    this.Data = null;
    this.Length = 0;
    ...
    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian != isDataInLittleEndianFormat){
        GuitPySynth.Util.WaveHelper.SwapEndianess(this.Data, bits);
    }
};
GuitPySynth.Util.PcmData.prototype = {
    get_BitsPerSample: function (){
        return this.BytesPerSample * 8;
    }
};
GuitPySynth.Util.PcmData.Create = function (bits, pcmData, isDataInLittleEndianFormat){
    switch (bits){
        case 8:
            return new GuitPySynth.Util.PcmData8Bit(bits, pcmData, isDataInLittleEndianFormat);
        case 16:
            return new GuitPySynth.Util.PcmData16Bit(bits, pcmData, isDataInLittleEndianFormat);
        case 24:
            return new GuitPySynth.Util.PcmData24Bit(bits, pcmData, isDataInLittleEndianFormat);
        case 32:
            return new GuitPySynth.Util.PcmData32Bit(bits, pcmData, isDataInLittleEndianFormat);
        default:
            throw $CreateException(new System.Exception.ctor$$String("Invalid PCM format. " + bits + "bit pcm data is not supported."), new Error());
    }
};
GuitPySynth.Util.PcmData8Bit = function (bits, pcmData, isDataInLittleEndianFormat){
    GuitPySynth.Util.PcmData.call(this, bits, pcmData, isDataInLittleEndianFormat);
};
GuitPySynth.Util.PcmData8Bit.prototype = {
    get_Item: function (index){
        return ((this.Data[index] / 255) * 2) - 1;
    }
};
$Inherit(GuitPySynth.Util.PcmData8Bit, GuitPySynth.Util.PcmData);
GuitPySynth.Util.PcmData16Bit = function (bits, pcmData, isDataInLittleEndianFormat){
    GuitPySynth.Util.PcmData.call(this, bits, pcmData, isDataInLittleEndianFormat);
};
GuitPySynth.Util.PcmData16Bit.prototype = {
    get_Item: function (index){
        index *= 2;
        return (((this.Data[index] | (this.Data[index + 1] << 8)) << 16) >> 16) / 32768;
    }
};
$Inherit(GuitPySynth.Util.PcmData16Bit, GuitPySynth.Util.PcmData);
GuitPySynth.Util.PcmData24Bit = function (bits, pcmData, isDataInLittleEndianFormat){
    GuitPySynth.Util.PcmData.call(this, bits, pcmData, isDataInLittleEndianFormat);
};
GuitPySynth.Util.PcmData24Bit.prototype = {
    get_Item: function (index){
        index *= 3;
        return (((this.Data[index] | (this.Data[index + 1] << 8) | (this.Data[index + 2] << 16)) << 12) >> 12) / 8388608;
    }
};
$Inherit(GuitPySynth.Util.PcmData24Bit, GuitPySynth.Util.PcmData);
GuitPySynth.Util.PcmData32Bit = function (bits, pcmData, isDataInLittleEndianFormat){
    GuitPySynth.Util.PcmData.call(this, bits, pcmData, isDataInLittleEndianFormat);
};
GuitPySynth.Util.PcmData32Bit.prototype = {
    get_Item: function (index){
        index *= 4;
        return (this.Data[index] | (this.Data[index + 1] << 8) | (this.Data[index + 2] << 16) | (this.Data[index + 3] << 24)) / 2.147484E+09;
    }
};
$Inherit(GuitPySynth.Util.PcmData32Bit, GuitPySynth.Util.PcmData);
GuitPySynth.Util.Tables = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Util.Tables._isInitialized = false;
    GuitPySynth.Util.Tables._envelopeTables = null;
    GuitPySynth.Util.Tables._semitoneTable = null;
    GuitPySynth.Util.Tables._centTable = null;
    GuitPySynth.Util.Tables._sincTable = null;
});
GuitPySynth.Util.Tables.EnvelopeTables = function (index){
    if (!GuitPySynth.Util.Tables._isInitialized)
        GuitPySynth.Util.Tables.Init();
    return GuitPySynth.Util.Tables._envelopeTables[index];
};
GuitPySynth.Util.Tables.SemitoneTable = function (index){
    if (!GuitPySynth.Util.Tables._isInitialized)
        GuitPySynth.Util.Tables.Init();
    return GuitPySynth.Util.Tables._semitoneTable[index];
};
GuitPySynth.Util.Tables.CentTable = function (index){
    if (!GuitPySynth.Util.Tables._isInitialized)
        GuitPySynth.Util.Tables.Init();
    return GuitPySynth.Util.Tables._centTable[index];
};
GuitPySynth.Util.Tables.SincTable = function (index){
    if (!GuitPySynth.Util.Tables._isInitialized)
        GuitPySynth.Util.Tables.Init();
    return GuitPySynth.Util.Tables._sincTable[index];
};
GuitPySynth.Util.Tables.Init = function (){
    var EnvelopeSize = 64;
    var ExponentialCoeff = 0.09;
    GuitPySynth.Util.Tables._envelopeTables = new Array(4);
    GuitPySynth.Util.Tables._envelopeTables[0] = (GuitPySynth.Util.Tables.RemoveDenormals(GuitPySynth.Util.Tables.CreateSustainTable(EnvelopeSize)));
    GuitPySynth.Util.Tables._envelopeTables[1] = (GuitPySynth.Util.Tables.RemoveDenormals(GuitPySynth.Util.Tables.CreateLinearTable(EnvelopeSize)));
    GuitPySynth.Util.Tables._envelopeTables[2] = (GuitPySynth.Util.Tables.RemoveDenormals(GuitPySynth.Util.Tables.CreateExponentialTable(EnvelopeSize, ExponentialCoeff)));
    GuitPySynth.Util.Tables._envelopeTables[3] = (GuitPySynth.Util.Tables.RemoveDenormals(GuitPySynth.Util.Tables.CreateSineTable(EnvelopeSize)));
    GuitPySynth.Util.Tables._centTable = GuitPySynth.Util.Tables.CreateCentTable();
    GuitPySynth.Util.Tables._semitoneTable = GuitPySynth.Util.Tables.CreateSemitoneTable();
    GuitPySynth.Util.Tables._sincTable = GuitPySynth.Util.Tables.CreateSincTable(16, 64, 0.43, GuitPySynth.Util.Tables.HammingWindow);
    GuitPySynth.Util.Tables._isInitialized = true;
};
GuitPySynth.Util.Tables.CreateSquareTable = function (size, k){
    ...
    for (var x = 0; x < size; x++){
        ...
        squaretable[x] = GuitPySynth.Synthesis.SynthHelper.ClampF((FourOverPi * value), -1, 1);
        phase += inc;
    }
    return squaretable;
};
GuitPySynth.Util.Tables.CreateCentTable = function (){
    //-100 to 100 cents
...
GuitPySynth.Util.Tables.CreateSemitoneTable = function (){
...
GuitPySynth.Util.Tables.CreateSustainTable = function (size){
...
GuitPySynth.Util.Tables.CreateLinearTable = function (size){
    var table = new Float32Array(size);
    for (var x = 0; x < size; x++){
        table[x] = x / (size - 1);
    }
...
GuitPySynth.Util.Tables.CreateExponentialTable = function (size, coeff){
    coeff = GuitPySynth.Synthesis.SynthHelper.ClampF(coeff, 0.001, 0.9);
...
GuitPySynth.Util.Tables.CreateSineTable = function (size){
    var graph = new Float32Array(size);
...
GuitPySynth.Util.Tables.RemoveDenormals = function (data){
    for (var x = 0; x < data.length; x++){
...
GuitPySynth.Util.Tables.VonHannWindow = function (i, size){
    return (0.5 - 0.5 * Math.cos(6.28318530717958 * (0.5 + i / size)));
};
GuitPySynth.Util.Tables.HammingWindow = function (i, size){
    return (0.54 - 0.46 * Math.cos(6.28318530717958 * i / size));
};
GuitPySynth.Util.Tables.BlackmanWindow = function (i, size){
    return (0.42659 - 0.49656 * Math.cos(6.28318530717958 * i / size) + 0.076849 * Math.cos(12.5663706143592 * i / size));
};
GuitPySynth.Util.Tables.CreateSincTable = function (windowSize, resolution, cornerRatio, windowFunction){
...
GuitPySynth.Util.SynthConstants = function (){
};
$StaticConstructor(function (){
    GuitPySynth.Util.SynthConstants.InterpolationMode = GuitPySynth.Bank.Components.Generators.Interpolation.Linear;
    GuitPySynth.Util.SynthConstants.AudioChannels = 2;
    GuitPySynth.Util.SynthConstants.Pi = 3.14159265358979;
    GuitPySynth.Util.SynthConstants.TwoPi = 6.28318530717958;
    GuitPySynth.Util.SynthConstants.HalfPi = 1.5707963267949;
    GuitPySynth.Util.SynthConstants.InverseSqrtOfTwo = 0.707106781186;
    GuitPySynth.Util.SynthConstants.DefaultLfoFrequency = 8;
    GuitPySynth.Util.SynthConstants.DefaultModDepth = 100;
    GuitPySynth.Util.SynthConstants.DefaultPolyphony = 40;
    GuitPySynth.Util.SynthConstants.MinPolyphony = 5;
    GuitPySynth.Util.SynthConstants.MaxPolyphony = 250;
    GuitPySynth.Util.SynthConstants.DefaultBlockSize = 64;
    GuitPySynth.Util.SynthConstants.MaxBufferSize = 0.05;
    GuitPySynth.Util.SynthConstants.MinBufferSize = 0.001;
    GuitPySynth.Util.SynthConstants.DenormLimit = 1E-38;
    GuitPySynth.Util.SynthConstants.NonAudible = 1E-05;
    GuitPySynth.Util.SynthConstants.SincWidth = 16;
    GuitPySynth.Util.SynthConstants.SincResolution = 64;
    GuitPySynth.Util.SynthConstants.MaxVoiceComponents = 4;
    GuitPySynth.Util.SynthConstants.DefaultChannelCount = 17;
    GuitPySynth.Util.SynthConstants.DefaultKeyCount = 128;
    GuitPySynth.Util.SynthConstants.DefaultMixGain = 0.35;
    GuitPySynth.Util.SynthConstants.MinVolume = 0;
    GuitPySynth.Util.SynthConstants.MaxVolume = 10;
    GuitPySynth.Util.SynthConstants.MinProgram = 0;
    GuitPySynth.Util.SynthConstants.MaxProgram = 127;
    GuitPySynth.Util.SynthConstants.MinPlaybackSpeed = 0.125;
    GuitPySynth.Util.SynthConstants.MaxPlaybackSpeed = 8;
});
GuitPySynth.Util.IOHelper = function (){
};
GuitPySynth.Util.IOHelper.ReadInt32LE = function (input){
...
GuitPySynth.Util.IOHelper.ReadUInt16LE = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    return ((ch2 << 8) | (ch1 << 0)&65535);
};
GuitPySynth.Util.IOHelper.ReadInt16LE = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    return GuitPySynth.Platform.TypeUtils.ToInt16((ch2 << 8) | (ch1 << 0));
};
GuitPySynth.Util.IOHelper.ReadInt32BE = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    var ch3 = input.ReadByte();
    var ch4 = input.ReadByte();
...
GuitPySynth.Util.IOHelper.ReadUInt16BE = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    return ((ch1 << 8) | (ch2 << 0)&65535);
};
GuitPySynth.Util.IOHelper.ReadInt16BE = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    return GuitPySynth.Platform.TypeUtils.ToInt16((ch1 << 8) | (ch2 << 0));
};
GuitPySynth.Util.IOHelper.ReadByteArray = function (input, length){
    var v = new Uint8Array(length);
    input.Read(v, 0, length);
    return v;
};
GuitPySynth.Util.IOHelper.Read8BitChars = function (input, length){
    var s = new Array();
...
GuitPySynth.Util.IOHelper.Read8BitString = function (input){
    var s = new Array();
...
GuitPySynth.Util.IOHelper.Read8BitStringLength = function (input, length){
    var s = new Array();
    var z = -1;
    for (var i = 0; i < length; i++){
        var c = input.ReadByte();
...
GuitPySynth.Util.IOHelper.ReadSInt8 = function (input){
    var v = input.ReadByte();
    return ((((v & 255) >> 7) * (-256)) + (v & 255));
};
GuitPySynth.Util.IOHelper.ReadUInt32 = function (input){
    var ch1 = input.ReadByte();
    var ch2 = input.ReadByte();
    var ch3 = input.ReadByte();
    var ch4 = input.ReadByte();
...
GuitPySynth.Util.IOHelper.ReadInt24 = function (input, index){
    var i;
    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
...
GuitPySynth.Util.IOHelper.ReadInt16 = function (input, index){
    if (GuitPySynth.Platform.TypeUtils.IsLittleEndian){
        return GuitPySynth.Platform.TypeUtils.ToInt16(input[index] | (input[index + 1] << 8));
    }
    else {
        return GuitPySynth.Platform.TypeUtils.ToInt16((input[index] << 8) | input[index + 1]);
    }
};
GuitPySynth.Util.WaveHelper = function (){
};
GuitPySynth.Util.WaveHelper.SwapEndianess = function (data, bits){
    bits /= 8;
    //get bytes per sample
    var swapArray = new Uint8Array(bits);
    for (var x = 0; x < data.length; x += bits){
        swapArray.set(data.subarray(x,x+bits),0);
        GuitPySynth.Platform.Std.Reverse(swapArray);
